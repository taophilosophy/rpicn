...
- **绝不能使用 deepl 进行任何英中翻译** deepl 有一个最大的问题就是 **吞字、吞行**。他会把他不理解的句式或者单词（比如使用了比喻义）直接无视掉。这个问题至少在 3 年前就存在。而且 deepl 对整句话的翻译往往呈现出相反的意思。起码对于树莓派官方文档和 FreeBSD 官方手册来说，都是如此。长文本翻译时，有时甚至会直接跳过五六行，无论网页版还是 API 都是如此。这对技术文档来说，是致命性的错误，因为 0 和 1 是对立的件事情，在这里不存在辩证法。
