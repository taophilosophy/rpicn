...
无论身在何处，均可通过 Raspberry Pi Connect 安全访问你的树莓派。

![hero](https://www.raspberrypi.com/documentation/services/images/hero.png)

要使用 Connect，请在你的树莓派上安装 Connect 软件。然后访问 connect.raspberrypi.com，在浏览器窗口中通过屏幕共享访问你的树莓派。你可以像将显示器插入树莓派一样使用桌面。

Connect 使用安全加密连接。默认情况下，Connect 直接在你树莓派和浏览器之间通信。但是，当 Connect 无法在你的树莓派和浏览器之间建立直接连接时，我们会使用位于伦敦的中继服务器。在后面情况下，raspberrypi.com 仅保留操作 Connect 所需的元数据。

## 安装

需要 64 位树莓派系统Bookworm 的树莓派才能运行 Connect，该 OS 使用 Wayland 窗口服务器。支持的设备型号为树莓派 5、树莓派 4 及树莓派 400。

>**注意**
>
>Connect 与树莓派系统Lite 或使用 X windows 服务器的系统并不兼容。 
...
可在你的树莓派上运行以下命令来安装 Connect：

```
$ sudo apt install rpi-connect
```

安装完成后，重启你的树莓派以启动 Connect 服务：
...
>**注意**
>
> 默认情况下，连接会在登录时自动启动。除非你从登录项中将其删除，否则无需手动启动连接。
...
现在你已经在你的树莓派上安装了 Connect，你必须将你的树莓派与你的 Raspberry Pi ID 关联起来才能使用 Connect。

如果你没有 Raspberry Pi ID，请创建一个。

### 链接你的树莓派和 Raspberry Pi ID

要将你的树莓派与你的 Raspberry Pi ID 绑定，请使用 Connect 生成验证链接。请访问该链接并登录你的 Raspberry Pi ID 以将你的树莓派绑定到你的账户。

你可以使用树莓派桌面上的 Connect 图标或命令行 rpi-connect 生成验证链接。
...
点击 Connect 图标，从下拉菜单中选择“登录”。这将打开一个验证链接，你可以使用它将你的树莓派链接到你的 Raspberry Pi ID。

![sign in](https://www.raspberrypi.com/documentation/services/images/sign-in.png)

#### 通过命令行

使用以下命令生成一个链接，该链接将绑定你的树莓派到你的 Raspberry Pi ID：
...
访问任何设备上的链接，将你的树莓派与你的 Raspberry Pi ID 绑定。
...
验证后，会为你的树莓派分配一个名称。选择一个能帮助你识别设备的名称。单击“创建设备并登录”按钮继续。

![create device](https://www.raspberrypi.com/documentation/services/images/create-device.png)

你现在可以从你的树莓派进行屏幕共享。Connect 系统托盘图标将变为蓝色，表示你的树莓派已连接到 Connect 服务。你应该收到一封电子邮件通知，提示新设备已登录到 Connect。

![sign in email](https://www.raspberrypi.com/documentation/services/images/sign-in-email.png)

>**警告**
>
>如果收到电子邮件，称有一台你不认识的设备登录了 Connect，请立即更改你的 Raspberry Pi ID 密码。从 Connect 中移除该设备，以永久取消它与你的账户的绑定。请考虑启用双因素身份验证，以保护你的账户安全。


![linked with](https://www.raspberrypi.com/documentation/services/images/linked-with.png)

单击 Connect 系统托盘图标以打开 Connect 菜单。该菜单显示你当前的登录状态，登出选项，以及启用或禁用屏幕共享的选项。
...
现在你的树莓派已在 Connect 网站中列出，你可以在任何地方仅用浏览器来共享你的树莓派屏幕。

在计算机上访问 connect.raspberrypi.com。

Connect 会将你重定向到 Raspberry Pi ID 服务以进行登录。登录后，Connect 会显示已链接设备的列表。可用于屏幕共享的设备在设备名称下方显示灰色的屏幕共享标签。

![list of devices](https://www.raspberrypi.com/documentation/services/images/list-of-devices.png)

单击你想要屏幕共享的设备右侧的“Connect”按钮。这将打开浏览器窗口，显示你的树莓派的桌面。

![connecting](https://www.raspberrypi.com/documentation/services/images/connecting.png)

你现在可以像在本地一样使用你的树莓派。有关 Connect 的更多信息，请将鼠标悬停在断开按钮右侧的挂锁图标上。
...
要关闭屏幕共享，请单击连接系统托盘图标，取消选择允许屏幕共享。连接系统托盘图标的下部将变为灰色。你仍然登录到连接，但你的屏幕无法共享。
...
连接列出与你的 Raspberry Pi ID 绑定的所有树莓派。每个设备都有一个“连接”按钮，用于打开屏幕共享会话（如果该设备已启用屏幕共享）。每个设备的设备名称下都有一组标签，指示可用服务。

![list of devices](https://www.raspberrypi.com/documentation/services/images/list-of-devices.png)

单击设备名称以打开设备详细信息页面。此屏幕提供有关你的设备的低级信息。你还可以编辑设备名称或从“Connect”中删除设备。
...
卸载后，树莓派的序列号仍与你的 Raspberry Pi ID 绑定。设备仍然显示在 Connect 网站中，但无法用于屏幕共享。如果你在同一台树莓派上再次安装 Connect，即使使用不同的 SD 卡，它也将在 Connect 门户中复用现有设备名称。
...
* Connect 仅支持共享树莓派的单个主显示屏。当树莓派连接到多个 HDMI 屏幕时，Connect 有时会共享辅助屏幕的内容。你可以通过右键单击桌面并在“桌面首选项”中更改任务栏位置来解决此问题。
* Connect 不支持屏幕键盘。为了实现完整功能，请使用物理键盘。
* Connect 需要使用实现了 ECMAScript 2020（ES11）的浏览器，因为它使用了旧版浏览器中不可用的功能。
* 浏览器会拦截某些按键和组合键。因此，你无法将这些键输入到你的 Connect 窗口中。Connect 提供了工具栏来模拟一些最常用的被拦截的按键。

### 启用增强型日志记录

你可以为 rpi-connect 和其专用的 WayVNC 服务器启用调试日志，以详细记录树莓派上的本地操作。
