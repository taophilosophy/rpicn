...
树莓派 4 有两个 HDMI，对于许多与 HDMI 相关的 config.txt 命令，有必要指定所指的 HDMI 端口。HDMI 条件过滤器将后续 HDMI 配置限制到特定端口。
...
>**重要**
>
>与 x86 机器上的 GPU 不同，增加内存并不能提高 3D 性能，VideoCore 的架构意味着指定比必要值更大的值不会带来性能优势，事实上可能会损害性能。

为了确保 Linux 的最佳性能，您应将 gpu_mem 设置为可能的最低值。如果某个特定的图形功能无法正常工作，请尝试增加 gpu_mem 的值，注意下面显示的推荐最大值。

在树莓派 4 上，GPU 的 3D 组件具有自己的内存管理单元（MMU），不使用 gpu_mem 分配的内存。相反，在 Linux 内动态分配内存。这使得在树莓派 4 上可以指定比以前型号更小的 gpu_mem 值。
...
>**重要**
>
>Raspberry Pi OS 上的相机堆栈（libcamera）使用 Linux CMA 内存来分配缓冲区，而不是使用 GPU 内存，因此增加 GPU 内存大小没有任何好处。
...
>**注意**
>
>因为树莓派 4 和树莓派 400 有两个 HDMI 端口，一些 HDMI 命令可以应用于任一端口。您可以使用语法 <command>:<port> ，其中端口为 0 或 1，以指定设置应适用于哪个端口。如果未指定端口，则默认为 0。如果在不需要端口号的命令上指定端口号，则端口将被忽略。有关语法和替代机制的更多详细信息，请参阅文档的条件部分中 HDMI 子部分。 
...
>**注意**
>
>在树莓派 4 上，设置 hdmi_blanking=1 不会导致 HDMI 输出关闭，因为此功能尚未实现。当使用不使用 framebuffer 的应用程序时，此功能可能会导致问题，例如 omxplayer 。
...
树莓派 4 无法在 1366×768 @ 60Hz 的 HDMI 上输出。在某些显示器上，可以配置它们使用 1360×768 @ 60Hz。它们通常不通过 EDID 广告此模式，因此无法自动进行选择，但可以通过手动添加来选择：
