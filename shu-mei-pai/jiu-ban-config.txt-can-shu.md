...
若文本超出屏幕左边缘，请增加此值；若屏幕左边缘和文本之间有黑色边框，请调低此值。

#### `overscan_right`

参数 `overscan_right` 可指定将添加到屏幕右边缘过扫描的固件默认值的像素数。默认值为 `0`。

若文本超出屏幕右边缘，请增加此值；若屏幕右边缘和文本之间有黑色边框，请减少此值。
...
>**注意**
>
>通常不建议使用此功能：因为这样做，显示器上的所有图层都将由 GPU 缩放，可能会降低图像质量。建议针对显示器自身禁用过扫描，以避免图像被 GPU 和显示器缩放两回。
...
若将 `framebuffer_ignore_alpha` 置为 `1`，会禁用 alpha 通道。可以改善 32 位 `framebuffer_depth` 显示。
...
此配置条目设置可创建的固件帧缓冲区的最大数量。有效值为 `0`、`1` 和 `2`。在旧设备上，默认设置为 `1`，因此当使用多个显示器时（如 HDMI、DSI、DPI 显示器）时，需要将其增加到 `2`。树莓派 4 的默认配置即是 `2`，因为它有两个 HDMI 端口。

在大多数情况下将其设置为 `2` 是安全的，因为只有在实际检测到连接的设备后才会创建帧缓冲区。
...
请注意，旋转 90 度和 270 度需要更多的 GPU 内存，因此这些参数在 16MB GPU 分配下不可用。

你可以组合将旋转、镜像来将它们叠加。你也可以用相同的方式同时进行水平和垂直镜像。例如，旋转 180 度，加上垂直和水平镜像为 `0x20000 + 0x10000 + 2 = 0x30002`。
...
树莓派 4 无法在 HDMI 输出 `1366×768 @ 60Hz`。对于某些显示器，可配置它们使用 `1360×768 @ 60Hz`。它们通常不由 EDID 广播此信号格式，因此无法自动选定，但可以手动添加来选定：
