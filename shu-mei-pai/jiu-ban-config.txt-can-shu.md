...
>**重要**
>
>与 x86 设备上的 GPU 不同，提高显存并不能改善 3D 性能。VideoCore 的架构意味着：**指定比必要值更大的值不仅不能带来性能上的优势**，相反还可能会降低性能。
...
在旧内核上，分配给 GPU 的内存用于显示器、3D、编解码器和摄像头等用途，以及一些基本固件管理。下面指定的最大值假定你正使用着以上全部的功能。如无，则应使用较小的 `gpu_mem` 值。
