...
这里所说的 config.txt 选项被视为旧版设置，不再被Raspberry Pi OS Bookworm 使用，且没有官方支持。它们要么涉及旧版软件，如固件图形驱动程序，已被弃用，或者只有极少数人可能会使用到。然而，它们仍然记录在此，因为它们对于使用旧版操作系统的用户或进行裸机开发的人仍然可能有益。
