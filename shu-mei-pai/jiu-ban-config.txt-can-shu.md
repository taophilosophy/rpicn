...
如果设置为非 `0` 值，则将：为加载的文件（内核、initramfs、设备树 .dtb 文件和叠加层）记录 SHA256 哈希日志，哈希值由工具 `sha256sum` 生成。如启用，记录输出将发送到串口，亦可通过 `sudo vclog --msg` 获取。在调试启动故障时，此参数可能极有用，但可能会增加启动时间（秒）。在所有平台上，默认值均为 `0`。
