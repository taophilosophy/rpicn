...
若将 `arm_peri_high` 置为 `1` 可在树莓派 4 上启用高性能外设模式。若加载了对应的 DTB，将会自动设定。

>**注意**
>
>若在缺失兼容设备树的情况下，启用高性能外设模式，会导致系统无法启动。目前的 ARM stub 支持尚不完善，因此你还需使用 `armstub` 加载对应的文件。


### `kernel_address`

`kernel_address` 是内核镜像将要加载的内存地址。在默认情况下，32 位内核会加载到地址 `0x8000`，64 位内核会加载到地址 `0x200000`。若设置了 `kernel_old`，内核会加载到地址 `0x0`。
