...
armstub 是从中加载 ARM stub 的引导分区上的文件名。默认的 ARM 存根存储在固件中，并根据树莓派型号和各种设置自动选择。

stub 是在内核之前运行的一小段 ARM 代码。其工作是在将控制权传递给内核之前设置底层硬件，如中断控制器。
...
>**注意**
>
>在没有兼容设备树的情况下启用高外设模式会导致系统无法启动。目前缺乏 ARM stub 支持，因此你还需要使用 armstub 加载适当的文件。
...
如果设置为非 0，则启用为加载的文件（内核、initramfs、设备树 .dtb 文件和叠加层）记录 SHA256 哈希的日志，该哈希由 sha256sum 实用程序生成。如果启用，记录输出将发送到 UART，并且也可通过 sudo vclog --msg 访问。在调试启动问题时，此参数可能很有用，但可能会增加启动时间数秒。在所有平台上，默认值为 0。
...
在树莓派 2、3 上，设置此参数将禁用 GPU 进入超频模式。可在特定负载下启用。
...
树莓派 4 有两个 HDMI 端口，同 HDMI 相关的 config.txt 命令有许多，有必要指定所针对的 HDMI 端口。HDMI 条件筛选器将后续 HDMI 配置限制到特定端口。
...
为了确保 Linux 维持最佳性能，你应尽可能的把 gpu_mem 设置为最低值。如果某个特定的图形功能无法正常工作，请尝试提高 gpu_mem 的值，注意下面显示的推荐最大值。

在树莓派 4 上，GPU 的 3D 组件有自己的内存管理单元（MMU），不使用 gpu_mem 分配的内存。取而代之的是在 Linux 内动态分配内存。这使得在树莓派 4 上可以指定比以前型号更小的 gpu_mem 值。

在旧版内核上，分配给 GPU 的内存用于显示、3D、编解码器和摄像头等用途，以及一些基本固件管理。下面指定的最大值假定你正在使用所有这些功能。如果没有使用，则应使用较小的 gpu_mem 值。
...
>**注意**
>
>因为树莓派 4、400 有两个 HDMI 端口，某些 HDMI 命令可以应用于特定接口。你可以使用语法 `<命令>:<端口>`，其中端口为 0 或 1，以指定设置应适用于哪个端口。如果未指定端口，则默认为 0。如果在不需要端口号的命令上指定端口号，则端口将被忽略。有关语法和替代机制的更多详细信息，请参阅文档的条件部分中 HDMI 子部分。

#### `hdmi_safe`

将 hdmi_safe 设置为 1 将触发使用“安全模式”，尝试以最大的 HDMI 兼容性启动。这与设置以下参数相同：
...
固件和 KMS 使用像素频率来筛选 HDMI 模式。请注意，这与帧率不同。它指定了有效模式可以具有的最大频率，从而剔除更高频率的模式。因此，如果你希望禁用所有 4K 模式，你可以指定最大频率为 200000000，因为所有 4K 模式的频率都大于此值。
...
此配置条目设置可创建的固件帧缓冲区的最大数量。有效参数为 0、1 和 2。在旧版的设备上，默认设置为 1，因此在使用多个显示器时（例如 HDMI、DSI、DPI 显示器），需要将其增加到 2。树莓派 4 的配置将此默认设置为 2，因为它有两个 HDMI 端口。
...
请注意，90 度和 270 度旋转参数需要 GPU 上额外的内存，因此这些参数在 16MB GPU 分配下不可用。
