...
这里描述的 config.txt 选项被视为旧版设置，不被 Raspberry Pi OS Bookworm 使用，并不再得到官方支持。它们要么涉及旧版软件，如固件图形驱动程序，已被弃用，或者大多数人极不可能使用。然而，它们仍然在此记录，因为它们对于使用旧版操作系统的用户或进行裸机开发的人仍然可能有益。
...
在 Raspberry Pi 4 上，如果存在文件 start4x.elf 和 fixup4x.dat ，则将使用这些文件。
...
| WARNING | 此设置已弃用。请改用 arm_64bit 以启用 64 位内核。 |
| --------- | --------------------------------------------------- |
...
将 arm_peri_high 设置为 1 可以在 Raspberry Pi 4 上启用高外设模式。如果加载了适当的 DTB，则会自动设置。

| NOTE | 在没有兼容的设备树的情况下启用高外设模式会导致系统无法启动。目前缺少 ARM 存根支持，因此您还需要使用 armstub 加载适当的文件。 |
| ------ | ------------------------------------------------------------------------------------------------------------------------------ |
...
init_uart_clock 是初始 UART 时钟频率。默认值为 48000000 （48MHz）。请注意，此时钟仅适用于 UART0（Linux 中的 ttyAMA0），且 UART 的最大波特率受限于时钟的 1/16。Raspberry Pi 3 和 Raspberry Pi Zero 上的默认 UART 是 UART1（Linux 中的 ttyS0），其时钟为核心 VPU 时钟 - 至少 250MHz。

### `bootcode_delay`

bootcode_delay 命令在 bootcode.bin 秒内延迟，然后加载 start.elf ：默认值为 0 。

这在在读取显示器的 EDID 之前插入延迟特别有用，例如，如果 Raspberry Pi 和显示器来自同一电源，但显示器启动时间比 Raspberry Pi 更长。如果在初始启动时显示检测错误，但在不断电的情况下重新启动 Raspberry Pi 时正确，请尝试设置此值。
...
### enable_gic （仅适用于 Raspberry Pi 4）

在 Raspberry Pi 4B 上，如果将此值设置为 0 ，则中断将通过旧版中断控制器路由到 Arm 内核，而不是通过 GIC-400。默认值为 1 。
...
设置 uart_2ndstage=1 会导致第二阶段加载程序（在 Raspberry Pi 4 之前的设备上为 bootcode.bin ，在 Raspberry Pi 4 设备上为 EEPROM 中的引导代码）和主固件（ start*.elf ）将诊断信息输出到 UART0。
...
设置 enable_jtag_gpio=1 选择 GPIO 引脚 22-27 的 Alt4 模式，并设置一些内部 SoC 连接，启用 Arm CPU 的 JTAG 接口。它适用于所有型号的 Raspberry Pi。
...
在 Raspberry Pi 2 和 3 上，设置此标志将禁用 GPU 进入 turbo 模式，该模式在特定负载下可以启用。
...
| NOTE | 此过滤器仅适用于 Raspberry Pi 4。 |
| ------ | ----------------------------------- |

Raspberry Pi 4 有两个 HDMI 端口，对于许多与 HDMI 相关的 config.txt 命令，有必要指定所指的 HDMI 端口。HDMI 条件过滤器将后续 HDMI 配置限制到特定端口。
...
| NOTE | Raspberry Pi 5 不会代表操作系统分配 GPU 内存，因此以下设置不起作用。 |
| ------ | ---------------------------------------------------------------------- |

### `gpu_mem`

指定要为 GPU 专用保留多少兆字节的内存：剩余内存分配给 Arm CPU 供操作系统使用。对于内存少于 1GB 的 Raspberry Pi，默认值为 64 ；对于内存为 1GB 或更多的 Raspberry Pi，默认值为 76 。

| IMPORTANT | 与 x86 机器上的 GPU 不同，增加内存并不能提高 3D 性能，VideoCore 的架构意味着指定比必要值更大的值不会带来性能优势，事实上可能会损害性能。 |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------ |

为了确保 Linux 的最佳性能，您应将 gpu_mem 设置为可能的最低值。如果某个特定的图形功能无法正常工作，请尝试增加 gpu_mem 的值，注意下面显示的推荐最大值。

在 Raspberry Pi 4 上，GPU 的 3D 组件具有自己的内存管理单元（MMU），不使用 gpu_mem 分配的内存。相反，在 Linux 内动态分配内存。这使得在 Raspberry Pi 4 上可以指定比以前型号更小的 gpu_mem 值。
...
| 256MB      | `128`                                 |
| 512MB      | `384`                                 |
| 1GB 或更大 | 在 Raspberry Pi 4 上， 512 ， 76 |

| IMPORTANT | Raspberry Pi OS 上的相机堆栈（libcamera）使用 Linux CMA 内存来分配缓冲区，而不是使用 GPU 内存，因此增加 GPU 内存大小没有任何好处。 |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------ |

可以将 gpu_mem 设置为较大的值，但应避免这样做，因为可能会导致问题，比如阻止 Linux 启动。最小值为 16 ，但这会禁用某些 GPU 功能。

您还可以使用 gpu_mem_256 ， gpu_mem_512 和 gpu_mem_1024 ，以便在不同内存量的 Raspberry Pi 之间交换相同的 SD 卡，而无需每次都编辑 config.txt ：

### `gpu_mem_256`

gpu_mem_256 命令为具有 256MB 内存的 Raspberry Pi 设置以兆字节为单位的 GPU 内存。如果内存大小不是 256MB，则会被忽略。这会覆盖 gpu_mem 。

### `gpu_mem_512`

gpu_mem_512 命令设置了 Raspberry Pi 内存为 512MB 时的 GPU 内存（以兆字节为单位）。如果内存大小不是 512MB，则会被忽略。这将覆盖 gpu_mem 。

### `gpu_mem_1024`

gpu_mem_1024 命令设置了 Raspberry Pi 内存为 1GB 或更多时的 GPU 内存（以兆字节为单位）。如果内存大小小于 1GB，则会被忽略。这将覆盖 gpu_mem 。

### `disable_l2cache`

将此设置为 1 会禁用 CPU 对 GPU 的 L2 缓存的访问，并需要相应的禁用 L2 缓存的内核。在 BCM2835 上的默认值为 0 。在 BCM2836、BCM2837、BCM2711 和 BCM2712 上，ARM 有自己的 L2 缓存，因此默认值为 1 。标准的 Raspberry Pi kernel.img 和 kernel7.img 版本反映了缓存设置的差异。
...
| NOTE | 因为 Raspberry Pi 4 和 Raspberry Pi 400 有两个 HDMI 端口，一些 HDMI 命令可以应用于任一端口。您可以使用语法 <command>:<port> ，其中端口为 0 或 1，以指定设置应适用于哪个端口。如果未指定端口，则默认为 0。如果在不需要端口号的命令上指定端口号，则端口将被忽略。有关语法和替代机制的更多详细信息，请参阅文档的条件部分中 HDMI 子部分。 |
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
在 Raspberry Pi 4B 上，您可以使用 hdmi_edid_filename 命令指定要使用的 EDID 文件的文件名，并指定要应用文件的端口。这还需要 hdmi_edid_file=1 来启用 EDID 文件。
...
将 hdmi_ignore_cec_init 设置为 1 将阻止在启动过程中发送初始活动源消息。这可以防止在重新启动 Raspberry Pi 时，CEC 启用的电视机从待机状态中唤醒并切换频道。
...
| NOTE | 在 Raspberry Pi 4 上，设置 hdmi_blanking=1 不会导致 HDMI 输出关闭，因为此功能尚未实现。当使用不使用 framebuffer 的应用程序时，此功能可能会导致问题，例如 omxplayer 。 |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
如果您遇到 HDMI 问题（斑点，干扰），请尝试 7 。非常长的 HDMI 电缆可能需要高达 11 ，但除非绝对必要，否则不应使用这么高的值。

旧版 Raspberry Pi 4 上会忽略此选项。
...
| 68        | 720p           | 50 赫兹  | 64:27    | 树莓派 4           |
| 69        | 720p           | 60Hz     | 64:27    | 树莓派 4           |
| 70        | 720p           | 100 赫兹 | 64:27    | 树莓派 4           |
...
| 15        | 1024x768  | 43Hz     | 4:3        | 与 Raspberry Pi 不兼容 |
| 16        | 1024x768  | 60 赫兹  | 4:3        |                        |
| 17        | 1024x768  | 70 赫兹  | 4:3        |                        |
| 18        | 1024x768  | 75Hz     | 4:3        |                        |
| 19        | 1024x768  | 85 赫兹  | 4:3        |                        |
| 20        | 1024x768  | 120 赫兹 | 4:3        |                        |
| 21        | 1152x864  | 75Hz     | 4:3        |                        |
| 22        | 1280x768  | 60Hz     | 15:9       | 降低清屏时间（reduced blanking）               |
| 23        | 1280x768  | 60 赫兹  | 15:9       |                        |
| 24        | 1280x768  | 75 赫兹  | 15:9       |                        |
...
| 39        | 1360x768  | 60 赫兹  | 16:9       |                        |
| 40        | 1360x768  | 120 赫兹 | 16:9       | 降低清屏时间（reduced blanking）               |
| 41        | 1400x1050 | 60Hz     | 4:3        | 降低清屏时间（reduced blanking）               |
| 42        | 1400x1050 | 60Hz     | 4:3        |                        |
| 43        | 1400x1050 | 75Hz     | 4:3        |                        |
| 44        | 1400x1050 | 85 赫兹  | 4:3        |                        |
| 45        | 1400x1050 | 120 赫兹 | 4:3        | 降低清屏时间（reduced blanking）               |
...
| 81        | 1366x768  | 60Hz     | 16:9       | [不适用于 Raspberry Pi 4 和 5](https://www.raspberrypi.com/documentation/computers/config_txt.html#hdmi-pipeline-for-raspberry-pi-4-and-5)                       |
...
| NOTE | 旧版树莓派 4 之前的型号支持的最高模式为 1920×1200，带有降低清屏时间（reduced blanking）的 60Hz，而树莓派 4 可以支持高达 4096×2160（口语上称为 4k）的 60Hz。还请注意，如果您正在使用树莓派 4 的两个 HDMI 端口进行 4k 输出，则两者的输出都将限制在 30Hz。 |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
* 将输出格式设置为 VGA 60Hz ( hdmi_group=1 和 hdmi_mode=1 ) 并启动您的 Raspberry Pi
...
请注意，这只是创建模式（第 2 组模式 87）。要使 Raspberry Pi 默认使用此模式，您必须添加一些附加设置。例如，要选择 800×480 分辨率并启用音频驱动：
...
指定 Raspberry Pi 触摸显示屏的帧率，单位为 Hz/fps。默认为 60Hz。
...
对于旧版图形驱动程序（适用于早于 Raspberry Pi 4 的型号），请使用 display_lcd_rotate 旋转或翻转 LCD 方向。参数与 display_hdmi_rotate 相同。另请参阅 lcd_rotate 。
...
## 旧版 Raspberry Pi 4 HDMI 管道

| IMPORTANT | 当使用 VC4 KMS 图形驱动程序时，完整的显示管道由 Linux 管理 - 包括 HDMI 输出。这些设置仅适用于旧版 FKMS 和基于固件的图形驱动程序。 |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------- |

Raspberry Pi 4 无法在 1366×768 @ 60Hz 的 HDMI 上输出。在某些显示器上，可以配置它们使用 1360×768 @ 60Hz。它们通常不通过 EDID 广告此模式，因此无法自动进行选择，但可以通过手动添加来选择：
