...
这里提供了代替参数 `start_file`、`fixup_file` 的简单方法，这是所推荐的固件配置的设定方法。
...
命令 `gpu_mem_256` 为搭载 256 \MB 内存的树莓派设置 GPU 内存（MB）。如果内存容量不是 256MB，则会被忽略。该参数会覆盖 `gpu_mem`。

### `gpu_mem_512`

命令 `gpu_mem_512` 为搭载 512 MB 内存的树莓派设置 GPU 内存（MB）。如果内存容量不是 512MB，则会被忽略。这将覆盖 `gpu_mem`。
