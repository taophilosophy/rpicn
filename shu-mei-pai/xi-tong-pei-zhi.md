...
在树莓派上，加载程序（如 `start.elf` 镜像）的工作是把叠加层与适当的基础设备树相结合，然后将完全解析的设备树传递给内核。基础设备树位于 FAT 分区（Linux 在 `/boot/firmware/`）中与 `start.elf` 相邻，命名为 `bcm2711-rpi-4-b.dtb`，`bcm2710-rpi-3-b-plus.dtb` 等等。请注意，某些型号（3A+，A，A+）将使用“b” 等效型号（3B+，B，B+），分别。此选择是自动的，并允许在各种设备中使用相同的 SD 卡镜像。
