...
如果你在阅读本文档后，仍未找到解决设备树问题的答案，也能得到帮助。通常可在 Raspberry Pi 论坛找到答案，特别是设备树论坛。
...
通过这个界面可以修改时钟配置，尽管结果很难预测！时钟系统的配置异常复杂。有五个独立的 PLL，每个 PLL 都有自己固定（或可变，如 PLLC）的 VCO 频率。然后，每个 VCO 都有许多不同的通道，可以使用不同的 VCO 频率分频。每个时钟目的地都可以配置为来自其中一个时钟通道，尽管源到目的地的映射受到限制，因此并非所有通道都可以路由到所有时钟目的地。
...
>- ② 根据 IANA（互联网名称与数字地址分配机构）规范，中国标准时区（北京时间）使用的是 Asia/Shanghai（上海）而非北京。只有极少数项目会在时区中出现北京（Asia/Beijing）。
