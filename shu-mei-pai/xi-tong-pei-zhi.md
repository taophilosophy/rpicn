...
将打开菜单，你可以在其中选择你的键盘布局。通常修改会立即生效，但可能需要重启。键入单个字母会跳转到列表中的该字母。
...
退出 raspi-config 。如果你进行了需要重启的更改， raspi-config 会提示你重启。首次实施更改时，最好重启。如果选择调整 SD 卡大小，重启可能需要比平常更长的时间。
...
选择选项 1 ， Boot/Auto login 。重启以使更改生效。
...
如果在树莓派系统中不存在 video 条目，则树莓派操作系统将使用 HDMI 连接的显示器的 EDID 自动选择基于 Linux 内核信息的显示器支持的最佳分辨率。在精简版树莓派系统或控制台模式中，你必须自定义 video 条目以控制分辨率和旋转。
...
>**注意**
>
>旧版的树莓派系统使用一个 wpa_supplicant.conf 文件，该文件可以放入 boot 文件夹中以配置无线网络设置。从树莓派系统 Bookworm 版本开始，不再提供该功能。
