...
你还可以指定在端口上允许或拒绝哪种服务。在此示例中，我们拒绝了 22 端口上的 TCP：
...
这将启用对可疑 ssh 活动的 Fail2ban 检查，包括系统日志检查，并在阻止活动之前给予进行六次重试机会。
...
>**注意**
>
>旧版的树莓派系统会使用一个文件叫 `wpa_supplicant.conf`，把该文件放入 boot 文件夹就可以配置无线网络设置。从树莓派系统 Bookworm 版本开始，该功能不再可用。
...
假如使用的有线网络 IP 范围是 10.x.x.x。你可将你的树莓派连接到该网络，并在使用另一个 IP 范围的独立网络上为无线客户端提供服务，如 192.168.x.x。
