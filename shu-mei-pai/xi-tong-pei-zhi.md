...
raspi-config 可帮助你配置你的树莓派。不同树莓派型号之间的可用选项可能有所不同。要打开配置工具，请运行以下命令：

```
$ sudo raspi-config
```

你应该看到在灰色框中出现了一个带有选项的蓝色屏幕：
...
raspi-config 可自动编辑 /boot/firmware/config.txt 和各种 Linux 配置文件。某些选项需要重新启动才能生效。如果你更改了其中任何一个， raspi-config 在退出时会要求你重新启动。
...
系统选项子菜单可让你对引导、登录和网络过程的各个部分进行配置更改，以及一些其他系统级别的更改。
