...
>**技巧**
>
>树莓派桌面用户可以在“首选项”>“树莓派配置”中使用此软件的图形化版本。但是，某些高级配置仅存在于 raspi-config。 

raspi-config 能帮助你配置树莓派。不同树莓派型号之间的能用的选项不尽相同。若要打开这个配置工具，请执行以下命令：
...
使用上、下箭头键能在可选条目之间进行移动，并对当前所选高亮显示。

按右箭头键或按 Tab 键可选择 `<Select>` 和 `<Finish>` 按钮。按左箭头键或按 Tab 键可返回选项。

raspi-config 可自动编辑 /boot/firmware/config.txt 以及各种 Linux 配置文件。某些选项需要重启才能生效：如果你更改了他们当中任何一个，在退出 raspi-config 时，会要求你重启。

>**技巧**
>
>在选项值所提供长长的列表中（比如时区城市列表），输入单个字母即可跳转到列表的对应部分。例如，输入 L 会跳转至里斯本。 

### 系统选项

系统选项子菜单可让你对引导、登录和网络过程的各个部分进行配置修改，以及一些其他系统级别的修改。
...
选择是启动到控制台还是桌面，以及在打开电源时是否自动登录到当前用户账户。
...
如果屏幕上显示的初始文本在屏幕边缘消失，可启用过扫描（overscan）调整边框。在某些显示器上，特别是监控器上，禁用过扫描会使图像充满整个屏幕并去除黑边。
...
启用或禁用使用 SSH 远程访问你的树莓派的终端。

SSH 能让你在另一台计算机远程访问树莓派的命令行。在默认情况下，SSH 处于禁用状态。在 SSH 文档页面上阅读更多关于使用 SSH 的信息。如果将你的树莓派直接连接到了公共网络，请不要启用 SSH，除非你为所有用户都设置了安全的密码。
...
如果你的树莓派型号支持，可以对 CPU 进行超频。超频潜力因个别树莓派设备而异，即使是同一型号也会有所不同。超频过高可能会导致不稳定。

>**警告**
>
>**超频可能会缩短你树莓派的使用寿命。** 如果在某个特定级别超频导致系统不稳定，请尝试降低超频的程度。在启动过程中按住 Shift 键可临时禁用超频。 
...
设置你的本地时区，从地区开始，然后选择一个城市，例如"Europe/London"。键入一个字母以跳转到列表中的该字母。

#### 键盘

打开一个菜单，你可以在其中选择你的键盘布局。更改通常会立即生效，但可能需要重新启动。键入一个字母以跳转到列表中的该字母。
...
将你的操作系统分区扩展到填满整个存储设备，为你提供更多用于文件的空间。重新启动你的树莓派以完成此操作。通常，树莓派系统在首次启动时运行此操作。如果你将你的操作系统克隆到容量大于原始设备的独立存储设备上，则此选项可能会很有用。
...
退出 raspi-config 。如果你进行了需要重新启动的更改， raspi-config 会提示你重新启动。首次实施更改时，最好重新启动。如果选择调整 SD 卡大小，重新启动可能需要比平常更长的时间。
...
在树莓派桌面上，打开“首选项”菜单，然后选择“屏幕配置”实用程序。你应该看到连接到树莓派的显示器的图形表示。右键单击要修改的显示器，然后选择一个选项。单击“应用”并关闭“屏幕配置”以保存更改。
...
你应该看到连接到树莓派的显示器的图形表示。右键单击要修改的显示器，然后选择一个选项。单击“应用”并关闭屏幕配置以保存更改。
...
如果你运行 Wayland 桌面合成器，你可以通过编辑你的主目录中的 .config/wayfire.ini 文件来设置自定义显示分辨率。编辑现有的 [output:<device>] 部分，或者如果不存在，则添加一个新的 [output:<device>] 部分以适用于你的显示设备。要更改显示分辨率，请添加一行 mode 。例如，以下示例显示了名为 HDMI-A-1 的设备以 1080p 60Hz 分辨率的配置：
...
树莓派系统为设置无线连接提供了图形用户界面（GUI）。树莓派系统Lite 的用户和无头机器的用户可以使用 APT 从命令行设置无线网络连接。

>**注意**
>
>从树莓派系统Bookworm 开始，Network Manager 是默认的网络配置工具。早期版本的树莓派系统使用 APT 和其他工具进行网络配置。 
...
使用箭头键选择“本地化选项”菜单项。选择 WLAN 国家选项。使用箭头键从下拉菜单中选择你的国家。按 Enter 键选择你的国家。
...
更改 cmdline.txt 后，只有在重启后才会生效。使用以下命令重启你的树莓派：
...
你可以通过 raspi-config 应用程序或命令行更改当前用户账户的密码：
...
此命令将删除用户以及其主目录。如果你想保留用户的主目录，请运行不带 -remove-home 选项的命令。

### 更改默认用户

要更改在启动时自动登录到你树莓派的用户，请运行以下命令：
...
你可以将外部硬盘、固态硬盘或 USB 闪存连接到树莓派上的任何 USB 端口，并挂载文件系统以访问其中存储的数据。

默认情况下，你的树莓派会自动挂载一些流行的文件系统，如 FAT、NTFS 和 HFS+，位于 /media/pi/<HARD-DRIVE-LABEL> 位置。

>**注意**
>
>树莓派系统Lite 不会自动挂载。

要设置存储设备，使其始终挂载到你选择的特定位置，必须手动挂载。

### 挂载存储设备

你可以将存储设备挂载到特定文件夹位置。通常在 /mnt 文件夹中执行此操作，例如 /mnt/mydisk 。请注意，文件夹必须为空。
...
树莓派使用挂载点 / 和 /boot/firmware/ 。你的存储设备将显示在此列表中，以及任何其他连接的存储设备。

使用 SIZE、LABEL 和 MODEL 列来识别指向存储设备的磁盘分区的名称。例如， sda1 。FSTYPE 列包含了文件系统类型。如果你的存储设备使用 exFAT 文件系统，请安装 exFAT 驱动程序：
...
如果你的存储设备使用 NTFS 文件系统，你将只能以只读方式访问它。如果你想要对设备进行写操作，可以安装 ntfs-3g 驱动程序：
...
创建一个目标文件夹，用作存储设备的挂载点。在这种情况下使用的挂载点名称是 mydisk 。你可以指定自己喜欢的名称：
...
你可以修改 fstab 文件，定义树莓派启动时存储设备将自动挂载的位置。在 fstab 文件中，磁盘分区由通用唯一标识符（UUID）标识。
...
用你在上述步骤中找到的文件系统类型替换 fstype ，例如： ntfs 。

如果文件系统类型为 FAT 或 NTFS，请在 nofail 之后立即添加 ,umask=000 - 这将允许所有用户完全读写存储设备上的每个文件。

现在你已在 fstab 中设置了一个条目，你可以启动带有或不带有存储设备的树莓派。在拔下设备之前，你必须关闭树莓派或手动卸载它。

>**注意**
>
>如果在树莓派启动时未连接存储设备，启动时间将额外需要 90 秒。你可以通过在 nofail 后立即添加 ,x-systemd.device-timeout=30 来缩短这个时间。这将把超时时间更改为 30 秒，这意味着系统只会等待 30 秒，然后放弃尝试挂载磁盘。
...
当树莓派关机时，系统会负责卸载存储设备，以便安全拔下。如果你想手动卸载设备，可以使用以下命令：
...
关闭在存储设备上有打开文件的任何程序。如果你有一个终端打开，请确保你不在存储设备挂载的文件夹中，或者在其子文件夹中。

如果你仍然无法卸载存储设备，你可以使用 lsof 工具检查哪个程序在设备上打开了文件。你需要首先安装 lsof ，使用 apt ：
...
Linux 内核在启动时接受一组命令行参数。在树莓派上，这个命令行在引导分区中的一个文件中定义，称为 cmdline.txt 。你可以使用任何文本编辑器编辑此文本文件。
...
早期版本的树莓派系统中使用的传统固件和 FKMS 显示模式不再受支持。取而代之的是，最近的 OS 版本使用 KMS（内核模式设置）。

如果在树莓派系统中不存在 video 条目，则树莓派操作系统将使用 HDMI 连接的显示器的 EDID 自动选择基于 Linux 内核信息的显示器支持的最佳分辨率。在树莓派系统Lite 或控制台模式中，你必须自定义 video 条目以控制分辨率和旋转。
...
此部分包含了你可以在内核命令行中使用的其他条目。此列表不是详尽的。
...
将 `<username>` 条目更改为以下内容，并将 <username> 替换为你的用户名：
...
你还可以通过更改 sshd 配置来允许或拒绝特定用户。

```
$ sudo nano /etc/ssh/sshd_config
```

将以下行添加、编辑或追加到文件末尾，其中包含你希望允许登录的用户名：

```
AllowUsers alice bob
```

你还可以使用 DenyUsers 来明确阻止某些用户名登录：
...
Linux 有许多防火墙解决方案可用。大多数使用底层的 iptables 项目来提供数据包过滤。该项目位于 Linux netfiltering 系统之上。默认情况下，树莓派系统上安装了 iptables，但未设置。设置它可能是一项复杂的任务，一个比 iptables 提供更简单界面的项目是 Uncomplicated Firewall (UFW)。这是 Ubuntu 中的默认防火墙工具，可以安装到你的树莓派上：
...
你还可以指定在端口上允许或拒绝哪种服务。在此示例中，我们拒绝了端口 22 上的 TCP：

```
$ sudo ufw deny 22/tcp
```

即使你不知道服务使用哪个端口，也可以指定服务。此示例允许 ssh 服务通过防火墙访问：
...
规则可能相当复杂，允许阻止特定 IP 地址，指定允许流量的方向，或限制连接尝试的次数（例如，以帮助击败 DDoS 攻击）。你还可以指定要应用规则的设备（例如，eth0，wlan0）。请参阅 ufw 手册页（ man ufw ）以获取除下面命令之外的完整详细信息。
...
当将树莓派用作服务器时，你必须在防火墙中创建有意的漏洞以允许服务器流量。Fail2ban 可以帮助保护你的服务器。Fail2ban 检查日志文件并检查可疑活动，如多次暴力登录尝试。它可以帮助你避免手动检查入侵尝试的日志文件，然后通过 iptables 更新防火墙以阻止它们。
...
无头树莓派在没有显示器、键盘或鼠标的情况下运行。要无头运行树莓派，你需要一种从另一台计算机访问它的方式。要远程访问你的树莓派，你需要将树莓派连接到网络，并找到一种通过该网络访问树莓派的方式。

要将树莓派连接到网络，你可以通过以太网将设备插入有线连接，或配置无线网络。

要通过该网络访问树莓派，请使用 SSH。通过 SSH 连接，你就可以使用 raspi-config 来启用 VNC（如果你更喜欢图形桌面环境）。

如果你从头开始设置你的树莓派，请在刻录过程中设置无线网络和 SSH。如果你已经设置好了树莓派，你可以使用 raspi-config 配置 SSH。

>**警告**
>
>取决于你使用的树莓派型号和 SD 卡型号，你的树莓派可能需要长达五分钟的时间来启动并首次连接到无线网络。 

### 连接到有线网络

首次启动时，要连接到有线网络，请通过以太网将你的无头树莓派连接，或者如果你的树莓派没有以太网接口，则需使用以太网转换器。你的树莓派将自动连接到网络。

### 连接到无线网络

要在无头树莓派首次启动时配置无线网络访问，请使用 Raspberry Pi Imager 中的高级设置菜单。输入你首选无线网络的 SSID 和密码。你的树莓派将使用这些凭据在首次启动时连接到网络。某些无线适配器和某些树莓派开发板不支持 5GHz 网络；请查阅你的无线模块文档，以确保与你首选网络的兼容性。

>**注意**
>
>旧版的树莓派系统使用一个 wpa_supplicant.conf 文件，该文件可以放入 boot 文件夹中以配置无线网络设置。从树莓派系统Bookworm 版本开始，不再提供该功能。


### 远程访问

如果没有键盘或显示器，你需要一种方法来远程控制你的无头树莓派。在首次启动时，唯一的选项是 SSH。要在全新安装的树莓派系统上启用 SSH，请选择以下方法之一：
...
该文件应包含一行文本，由 `<username>:<password>` 组成：你想要使用的用户名，紧接着是一个冒号，然后是你想要使用的密码的加密表示。
...
你的树莓派可以使用无线模块主机自己的无线网络。如果你通过以太网端口（或第二个无线模块）将你的树莓派连接到互联网，连接到无线网络的其他设备可以通过你的树莓派访问互联网。

假如使用 10.x.x.x IP 块的有线网络。你可以将你的树莓派连接到该网络，并在使用另一个 IP 块的独立网络上为无线客户端提供服务，例如 192.168.x.x 。
...
要在命令行上创建托管的无线网络，请运行以下命令，将 `<example-network-name>` 和 `<example-password>` 占位符替换为你自己的值：

```
$ sudo nmcli device wifi hotspot ssid <example-network-name> password <example-password>
```

使用另一个无线客户端，如笔记本电脑或智能手机，连接到网络。查找一个 SSID 与 `<example-network-name>` 匹配的网络。输入你的网络密码，你应该成功连接到网络。如果你的树莓派通过以太网连接或第二个无线适配器访问互联网，你应该能够访问互联网。

### 禁用热点

要禁用热点网络并恢复将你的 Pi 用作无线客户端，请运行以下命令：
...
默认情况下，从你的树莓派托管的无线网络与通过以太网连接的父网络分开存在。在这种安排中，连接到父网络的设备无法直接与连接到你的树莓派托管的无线网络上的设备通信。如果你希望连接的无线设备能够与父网络上的设备通信，你可以将你的树莓派配置为网络桥接器。有了网络桥接器，连接到 Pi 托管的无线网络的每个设备都被分配一个父网络中的 IP 地址。

在下面的图表中，笔记本电脑存在于与路由器和有线客户端相同的 IP 块中：

![bridge network](../.gitbook/assets/bridge-network.png)

以下步骤描述了如何在你的树莓派上设置网络桥接，以实现无线客户端和父网络之间的通信。
...
最后，将你的无线热点连接添加到桥接中。你可以添加现有的热点接口或创建一个新的：
...
重启你的树莓派以使更改生效。现在你应该能够通过代理服务器访问互联网。
...
为了使用迷你 UART，你需要配置树莓派以使用固定的 VPU 核心时钟频率。这是因为迷你 UART 时钟与 VPU 核心时钟相连，因此当核心时钟频率发生变化时，UART 波特率也会发生变化。可以将 enable_uart 和 core_freq 设置添加到 config.txt 中，以更改迷你 UART 的行为。以下表总结了可能的组合：
...
反过来，由于平台设备只有在 DTB 请求时才会被创建，因此不再需要黑名单模块，这些模块过去是由板支持代码中定义的平台设备加载的结果。实际上，当前的树莓派系统映像不包含黑名单文件（除了一些 WLAN 设备，其中有多个可用驱动程序）。
...
树莓派开发板有两个 I2C 接口。这些通常分为：一个用于 ARM，一个用于 VideoCore（GPU）。在几乎所有型号上， i2c1 属于 ARM， i2c0 属于 VC，用于控制摄像头并读取 HAT EEPROM。然而，有两个早期版本的B 角色相反。
...
更改内核状态的命令变体（添加和移除内容）需要 root 权限，因此你可能需要在命令前加上 sudo 。只有在运行时应用的叠加和参数可以被卸载 - 固件应用的叠加或参数会被“烘焙”，因此不会被 dtoverlay 列出，也无法移除。
...
max_current 电源供应器可以提供的最大电流（以mA为单位）。固件报告由 USB-C、USB-PD 或 PoE 接口指示的值。对于台式电源供应器（例如连接到 GPIO 排针），请在引导加载程序配置中定义 PSU_MAX_CURRENT 以指示电源供应器的电流能力。
...
如果你有非常特定的需求，而默认的 DTB 不支持这些需求，或者你只是想尝试编写自己的 DT，请告诉加载程序加载替代的 DTB 文件，就像这样：
...
如果你阅读了本文档但未找到解决设备树问题的答案，可以获得帮助。作者通常可以在 Raspberry Pi 论坛上找到，特别是设备树论坛。
...
通过过程可能需要几秒钟。在此期间，GPIO 引脚可能不处于附加外围设备期望的状态（如 dt-blob.bin 或 config.txt 中定义）。由于不同的 GPIO 引脚具有不同的默认拉电阻，你应该针对你的外围设备执行以下操作之一：
...
* `function`
  ...
  * `emmc`
  * `arm_jtag`
* 驱动强度用于设置引脚的强度。请注意，你只能为该银行指定单个驱动强度。 <8> 和 <16> 是有效值。
...
这里有一些示例配置，你可以用来更改特定的时钟。当请求时钟配置时，我们将添加到此资源。
