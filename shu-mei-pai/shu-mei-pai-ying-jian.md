...
Raspberry Pi 推出了几个不同系列的计算机：

* 旗舰系列通常被简称为"Raspberry Pi"，提供高性能硬件、完整的 Linux 操作系统，以及一个大量常见ports的形式因子大致与信用卡大小相当。
* Zero 系列提供了一个完整的 Linux 操作系统和基本的ports，价格实惠，体积小，功耗低。
* Compute Module 系列，通常简称为 "CM"，提供高性能硬件和完整的 Linux 操作系统，适合工业和嵌入式应用的小型形态因素。Compute Module 模型拥有与相应旗舰型号相当的硬件，但较少的ports和没有板载 GPIO 引脚。用户需将 Compute Module 连接到一个提供给定应用所需ports和引脚的独立基板上。

此外，Raspberry Pi 还推出了 Pico 系列微型多用途微控制器板。Pico 型号不运行 Linux 或支持可移动存储，而是通过将二进制文件闪存到板载闪存存储器来进行编程。

### 旗舰系列

Model B 指示有一个带有 Ethernet port的模型。 Model A 指示一个更低成本的模型，体积更小，没有 Ethernet port，减少了 RAM，减少了 USB ports以限制板高度。

| Model                                                                                                                                                             | SoC                                    | 存储                     | GPIO              | 连通性                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | -------------------------- | ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![Raspberry Pi Model B](https://www.raspberrypi.com/documentation/computers/images/model-b.jpg?hash=caa7985f73e4fb3af8fb7b0a614d88b3)<br />树莓派模型 B                | [BCM2835](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2835)                                       | 256MB<br /><br />512MB           | 26 引脚 GPIO 头   | * HDMI* 2 × USB 2.0* CSI 相机 port* DSI 显示器 port* 3.5 毫米音频插孔* RCA 复合视频* 以太网（100Mb/s）* SD 卡插槽* 微型 USB 电源                                                                                                                                                                                                                 |
| ![Raspberry Pi Model A](https://www.raspberrypi.com/documentation/computers/images/model-a.jpg?hash=9b8ad338273f437641f74eb13a32adc1)<br />Raspberry Pi Model A        | [BCM2835](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2835)                                       | 256MB                    | 26 引脚 GPIO 头部 | * HDMI* USB 2.0* CSI 摄像头 port* DSI 显示 port* 3.5 毫米音频插孔* RCA 复合视频* SD 卡插槽* 微型 USB 电源                                                                                                                                                                                                                                         |
| ![Raspberry Pi Model B+](https://www.raspberrypi.com/documentation/computers/images/model-b-plus.jpg?hash=83f51a836116f3df1378346158d6e148)<br />Raspberry Pi Model B+ | [BCM2835](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2835)                                       | 512MB                    | 40 针 GPIO 引脚   | * HDMI* 4 × USB 2.0* CSI 摄像头 port* DSI 显示器 port* 3.5 毫米 AV 插孔* 以太网(100Mb/s)* microSD 卡槽* micro USB 电源                                                                                                                                                                                                                           |
| ![Raspberry Pi Model A+](https://www.raspberrypi.com/documentation/computers/images/model-a-plus.jpg?hash=e0df62b537945082df16ff45f453d2ad)<br />树莓派 A+模型         | [BCM2835](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2835)                                       | 256MB<br /><br />512MB           | 40 针 GPIO 头     | * HDMI* USB 2.0* CSI 摄像头 port* DSI 显示器 port* 3.5 毫米音视频插孔* microSD 卡槽* 微型 USB 供电                                                                                                                                                                                                                                                |
| ![Raspberry Pi 2 Model B](https://www.raspberrypi.com/documentation/computers/images/2-model-b.jpg?hash=f7ccdaf24be1e125198d6ef11a7d3258)<br />树莓派 2 型 B           | BCM2836（在版本 1.2 中切换到 BCM2837） | 1 GB                     | 40 针 GPIO 标头   | * HDMI* 4 × USB 2.0* CSI 摄像头 port* DSI 显示 port* 3.5 毫米 AV 插孔* 以太网 (100Mb/s)* microSD 卡插槽* 微型 USB 电源                                                                                                                                                                                                                           |
| ![Raspberry Pi 3 Model B](https://www.raspberrypi.com/documentation/computers/images/3-model-b.jpg?hash=26b673f0b2c427e9e29fada4336a3569)<br />树莓派 3 Model B        | [BCM2837](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2837)                                       | 1 GB                     | 40 针 GPIO 头     | * HDMI* 4 × USB 2.0* CSI 摄像头 port* DSI 显示器 port* 3.5 毫米 AV 插孔* 以太网（100Mb/s）* 2.4GHz 单频段 802.11n Wi-Fi（35Mb/s）* 蓝牙 4.1，蓝牙低功耗（BLE）* microSD 卡槽* 微型 USB 电源                                                                                                                                                      |
| ![Raspberry Pi 3 Model B+](https://www.raspberrypi.com/documentation/computers/images/3-model-b-plus.jpg?hash=55357b0ece66311f90f82db2dc09f3d2)<br />树莓派 3 型 B+    | [BCM2837b0](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2837b0)                                       | 1GB                      | 40 引脚 GPIO 标头 | * HDMI* 4 × USB 2.0* CSI 相机 port* DSI 显示 port* 3.5 毫米 AV 插孔* 支持 PoE 的以太网（300Mb/s）* 2.4/5GHz 双频 802.11ac Wi-Fi（100Mb/s）* 蓝牙 4.2，蓝牙低功耗（BLE）* microSD 卡槽* 微型 USB 电源                                                                                                                                             |
| ![Raspberry Pi 3 Model A+](https://www.raspberrypi.com/documentation/computers/images/3-model-a-plus.jpg?hash=e8035ce9e2c7f5d1c8050e039aaacbaa)<br />树莓派 3 型 A+    | [BCM2837b0](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2837b0)                                       | 512 MB                   | 40 引脚 GPIO 头   | * HDMI * USB 2.0 * CSI 相机 port * DSI 显示 port * 3.5 毫米 AV 插孔 * 2.4/5GHz 双频段 802.11ac Wi-Fi (100Mb/s) * 蓝牙 4.2、蓝牙低功耗 (BLE) * microSD 卡槽 * 微型 USB 电源                                                                                                                                                                        |
| ![Raspberry Pi 4 Model B](https://www.raspberrypi.com/documentation/computers/images/4-model-b.jpg?hash=f56bee1ea763677368e20dbc59574c85)<br />Raspberry Pi 4 Model B  | [BCM2711](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2711)                                       | 1GB<br /><br />2GB<br /><br />4GB<br /><br />8GB | 40 针 GPIO 引脚   | * 2 × 微型 HDMI* 2 × USB 2.0* 2 × USB 3.0* CSI 摄像头 port* DSI 显示屏 port* 3.5 毫米音视频插孔* PoE 支持的千兆以太网 (1Gb/s)* 2.4/5GHz 双频 802.11ac Wi-Fi (120Mb/s)* 蓝牙 5, 低功耗蓝牙 (BLE)* microSD 卡槽* USB-C 电源 (5V, 3A (15W))                                                                                                       |
| ![Raspberry Pi 400](https://www.raspberrypi.com/documentation/computers/images/400.jpg?hash=59a07dc791fc54f56c2416f42ced88e3)<br />Raspberry Pi 400                    | [BCM2711](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2711)                                       | 4GB                      | 40 引脚 GPIO 标题 | * 2× 微型 HDMI * USB 2.0 * 2× USB 3.0 * 千兆以太网（1Gb/s） * 2.4/5GHz 双频 802.11ac 无线网络（120Mb/s） * 蓝牙 5，蓝牙低功耗（BLE） * microSD 卡槽 * USB-C 电源（5V，3A（15W））                                                                                                                                                               |
| ![Raspberry Pi 5](https://www.raspberrypi.com/documentation/computers/images/5.jpg?hash=b888dab3bb8bcb8dd4e0541c99238eec)<br />树莓派 5                                | [BCM2712](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2712)                                       | 4GB<br /><br />8GB               | 40 引脚 GPIO 标题 | * 2 × 微型 HDMI* 2 × USB 2.0* 2 × USB 3.0* 2 × CSI 摄像头/DSI 显示ports* 单通道 PCIe FFC 连接器* UART 连接器* RTC 电池连接器* 四针 JST-SH PWM 风扇连接器* 支持 PoE+的千兆以太网（1Gb/s）* 2.4/5GHz 双频段 802.11ac Wi-Fi 5（300Mb/s）* 蓝牙 5，低功耗蓝牙（BLE）* microSD 卡槽* USB-C 电源（5V，5A（25W）或 5V，3A（15W），外设限制为 600mA） |

有关树莓派旗舰系列上ports的更多信息，请参阅原理图和机械图。

### 零系列

具有 H 后缀的型号在 GPIO 插头上预先焊接了排针。 不带 H 后缀的型号没有连接到 GPIO 插头的排针； 用户必须手动焊接排针或连接第三方排针套件。

所有零系列型号具有以下连接功能：

* 一个 microSD 卡槽
* 一个 CSI 摄像头 port (原始 Zero 的第 1.3 版引入了这个 port)
* 一个 mini HDMI port
* 2 × 微型 USB ports（一个用于输入电源，一个用于外部设备）

| Model                                                                                                                                                   | SoC | 存储器 | GPIO                          | 无线连接                                                             |
| --------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | -------- | ------------------------------- | ---------------------------------------------------------------------- |
| ![Raspberry Pi Zero](https://www.raspberrypi.com/documentation/computers/images/zero.jpg?hash=2d93a49cf668312604cfc00fc0660214)<br />树莓派 Zero             | [BCM2835](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2835)    | 512MB  | 40 针 GPIO 引脚头（未焊接）   | 无                                                                   |
| ![Raspberry Pi Zero W](https://www.raspberrypi.com/documentation/computers/images/zero-w.jpg?hash=3752f16ba033177e867614e87292076d)<br />树莓派 Zero W       | [BCM2835](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2835)    | 512MB  | 40 针 GPIO 引脚头（未焊接）   | * 2.4GHz 单频 802.11n Wi-Fi（35Mb/s）* 蓝牙 4.0，蓝牙低功耗（BLE）   |
| ![Raspberry Pi Zero WH](https://www.raspberrypi.com/documentation/computers/images/zero-wh.jpg?hash=f7d7c5c9b132395f45c308741fc85c7e)<br />树莓派 Zero WH    | [BCM2835](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2835)    | 512MB  | 40 针 GPIO 引脚头             | * 2.4GHz 单频段 802.11n Wi-Fi（35Mb/s）* 蓝牙 4.0，蓝牙低功耗（BLE） |
| ![Raspberry Pi Zero 2 W](https://www.raspberrypi.com/documentation/computers/images/zero-2-w.jpg?hash=5c934105e0a9be90fa0d506cac91a46f)<br />树莓派 Zero 2 W | [RP3A0](https://www.raspberrypi.com/documentation/computers/processors.html#rp3a0)    | 512MB  | 40 针 GPIO 引脚排针（未焊接） | * 2.4GHz 单频段 802.11n Wi-Fi (35Mb/s)* 蓝牙 4.2，蓝牙低功耗（BLE）  |

### 计算模块系列

| Model                                                                                                                                                                           | SoC | 存储器                   | 存储                                 | 外形尺寸                | 无线连接                                                                    |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | -------------------------- | -------------------------------------- | ------------------------- | ----------------------------------------------------------------------------- |
| ![Raspberry Pi Compute Module 1](https://www.raspberrypi.com/documentation/computers/images/compute-module-1.jpg?hash=725d6ee61d958098dc68dc7739deab88)<br />Raspberry Pi 计算模块 1 | [BCM2835](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2835)    | 512MB                    | 0GB（精简版）<br /><br />4GB                 | DDR2 SO-DIMM            | none                                                                        |
| ![Raspberry Pi Compute Module 3](https://www.raspberrypi.com/documentation/computers/images/compute-module-3.jpg?hash=3a5ff7c853190d7a07c51bf67c1082b8)<br />树莓派计算模块 3        | [BCM2837](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2837)    | 1GB                      | 0GB（精简版）<br /><br />4GB                 | DDR2 SO-DIMM            | 无                                                                          |
| ![Raspberry Pi Compute Module 3+](https://www.raspberrypi.com/documentation/computers/images/compute-module-3-plus.jpg?hash=352ed6d3402586078299193abe8ba754)<br />树莓派计算模块 3+ | [BCM2837b0](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2837b0)    | 1GB                      | 0GB（Lite）<br /><br />8GB<br /><br />16GB<br /><br />32GB   | DDR2 SO-DIMM            | 无                                                                          |
| ![Raspberry Pi Compute Module 4S](https://www.raspberrypi.com/documentation/computers/images/compute-module-4s.jpg?hash=4ec9821548515598adfe5a3cdc14789d)<br />树莓派计算模块 4S     | [BCM2711](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2711)    | 1GB<br /><br />2GB<br /><br />4GB<br /><br />8GB | 0GB（精简版）<br /><br />8GB<br /><br />16GB<br /><br />32GB | DDR2 SO-DIMM            | 无                                                                          |
...
| NOTE | 几个计算模块使用物理 DDR2 SO-DIMM 形态因子，但与 DDR2 SO-DIMM 电气规格不兼容。 |
| ------ | -------------------------------------------------------------------------------- |
...
带有 H 后缀的型号在 GPIO 引脚上预先焊接了排针。不带 H 后缀的型号不附带已连接到 GPIO 引脚的排针；用户必须手动焊接排针或连接第三方排针套件。

| Model                                                                                                                                                   | SoC | 存储       | 存储空间 | GPIO                      | 无线连接                                                             |
| --------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------------ | ---------- | --------------------------- | ---------------------------------------------------------------------- |
| ![Raspberry Pi Pico](https://www.raspberrypi.com/documentation/computers/images/pico.png?hash=950c52fb95a01a3eec3b225e71a14bc9)<br />树莓派 Pico             | [RP2040](https://www.raspberrypi.com/documentation/microcontrollers/rp2040.html#welcome-to-rp2040)    | 264 千字节 | 2MB      | 40 针 GPIO 头部（未安装） | 无                                                                   |
| ![Raspberry Pi Pico H](https://www.raspberrypi.com/documentation/computers/images/pico-h.png?hash=26d4f02827ffd2c911119d36da30bb27)<br />树莓派 Pico H       | [RP2040](https://www.raspberrypi.com/documentation/microcontrollers/rp2040.html#welcome-to-rp2040)    | 264 千字节 | 2MB      | 40 针 GPIO 接口（未焊接） | 无                                                                   |
| ![Raspberry Pi Pico W](https://www.raspberrypi.com/documentation/computers/images/pico-w.png?hash=ab356bc12db87ee6d8d3d62388baf4bd)<br />Raspberry Pi Pico W | [RP2040](https://www.raspberrypi.com/documentation/microcontrollers/rp2040.html#welcome-to-rp2040)    | 264 千字节 | 2MB      | 40 引脚 GPIO 头（未焊接） | * 2.4GHz 单频段 802.11n Wi-Fi（10Mb/s）* 蓝牙 5.2，蓝牙低功耗（BLE） |
| ![Raspberry Pi Pico WH](https://www.raspberrypi.com/documentation/computers/images/pico-wh.png?hash=a6be966c70758bbcd9f5115c4096cf2c)<br />树莓派 Pico WH    | [RP2040](https://www.raspberrypi.com/documentation/microcontrollers/rp2040.html#welcome-to-rp2040)    | 264 千字节 | 2MB      | 40 针 GPIO 引脚头         | * 2.4GHz 单频段 802.11n Wi-Fi（10Mb/s）* 蓝牙 5.2，蓝牙低功耗（BLE） |
...
| NOTE | 树莓派 3 型 A+ 的机械图也适用于树莓派 1 型 A+。 |
| ------ | ------------------------------------------------- |
...
所有 Raspberry Pi 产品均经过了广泛的合规测试。有关更多信息，请参阅产品信息门户。
...
由树莓派提供支持的 Powered by Raspberry Pi 计划，为希望使用树莓派 logo 形式的公司提供了一个流程，涵盖了搭载树莓派计算机或硅片的产品，以及由树莓派提供的服务。如果您希望开始申请流程，可以在线操作。

### 已批准的设计合作伙伴

我们的已批准设计合作伙伴名单提供了一组我们密切合作并支持的咨询公司，使它们能够在硬件、软件和机械领域提供付费设计服务。

## 频率管理和热控制

所有 Raspberry Pi 型号在重载情况下都会进行一定程度的热管理，以避免过热。SoC 具有内部温度传感器，GPU 上的软件会轮询以确保温度不超过我们定义的上限，所有型号均为 85°C。可以将此值设定为更低的数值，但不能设置为更高。当设备接近极限时，芯片（Arm，GPU）上使用的各种频率和有时电压会降低。这会减少产生的热量，保持温度控制在一定范围内。

当核心温度在 80°C 和 85°C 之间时，Arm 核心将逐渐降频。如果温度达到 85°C，那么 Arm 核心和 GPU 都将开始降频。

树莓派 3 Model B+ 采用了新的 PCB 技术，以提供更好的散热性能和增加热量质量。此外，引入了软温度限制，旨在最大化设备在达到硬限制 85°C 之前的 "冲刺" 时间。当达到软限制时，时钟速度从 1.4GHz 降至 1.2GHz，并略微降低操作电压。这减缓了温度升高的速率：我们以短暂的 1.4GHz 时间换取了更长时间的 1.2GHz 运行。默认情况下，软限制为 60°C，可以通过 config.txt 中的 temp_soft_limit 设置进行更改。
...
Raspberry Pi 4 设备实现了动态电压和频率调节（DVFS）。这种技术使得 Raspberry Pi 4 设备能在保持相同性能的同时运行在较低温度下。

SoC 内部的各种时钟（例如 Arm、Core、V3D、ISP、H264、HEVC）由固件监控，当它们未以最高速运行时，供给给这些时钟驱动的芯片部分的电压将相对于完全速度减少。事实上，只提供足够的电压以确保块以特定运行速度正常运行。这可以显著降低 SoC 的功耗，从而减少产生的总热量。

由于可能导致系统稳定性问题，特别是在使用电压降低固定时钟外设（例如 PCIe）时，存在潜在的系统稳定性问题，有三种 DVFS 模式可在 /boot/firmware/config.txt 中配置，具有以下特性。大多数系统应该使用 dvfs=3 ，无显示系统可能会因为较小的功耗降低而从 dvfs=1 中受益，但会面临 PCIe 稳定性问题的风险。

| 属性=值 | 描述                                                                                                                 |
| --------- | ---------------------------------------------------------------------------------------------------------------------- |
| `dvfs=1`        | 允许欠压                                                                                                             |
| `dvfs=2`        | 固定默认操作频率的电压                                                                                               |
| `dvfs=3`        | 根据需求增加超频电压（默认）。如果在 config.txt 中指定 over_voltage ，则会禁用动态电压调节，导致系统恢复到 dvfs=2 。 |

| NOTE | 该设置已在 Raspberry Pi 5 上移除，并且始终处于模式 3。 |
| ------ | -------------------------------------------------------- |

此外，还使用了更加分级的 CPU 调度器来更精细地控制 ARM 核频率，这意味着 DVFS 更为有效。现在的步进频率为 1500MHz、1000MHz、750MHz 和 600MHz。这些步进可以在 SoC 被节流时帮助，意味着几乎不会完全降至 600MHz，从而大大提高了全负载性能。
...
由于树莓派设备使用的 SoC 架构，以及在树莓派 OS 发行版中使用上游温度监控代码，基于 Linux 的温度测量可能不准确。然而， vcgencmd 命令能够提供准确且即时的当前 SoC 温度读数，因为它直接与 GPU 通信：
...
由于内建的节流功能，SoC 不需要散热片来防止过热损坏。不过，安装散热片或小型风扇可以减少热节流并提高性能。为了获得最佳气流和略微改善散热效果，建议将树莓派垂直安装。

### 风扇套件

树莓派 5 有两个官方风扇选项，用于辅助散热：

* [ 主动风冷器](https://www.raspberrypi.com/products/active-cooler/)
* [用于树莓派 5 的盒子](https://www.raspberrypi.com/products/raspberry-pi-5-case/)

这两者均插入位于板右上方在 40 个引脚 GPIO 标头和 USB 2 之间的四针 JST-SH PWM 风扇连接器中。风扇连接器从与 USB 外围设备相同的电流限制中提取电流。我们推荐为超频者使用主动风冷器盒子，因为它提供更好的冷却性能。

两种可用的官方配件都由 Raspberry Pi 固件积极管理。随着 Raspberry Pi 的温度升高，风扇的反应如下：

* 在 50°C 以下，风扇根本不转动（0%速度）
* 在 50°C 时，风扇以低速启动（30%速度）
* 在 60°C 时，风扇速度增加到中速（50% 速度）
* 在 67.5°C 时，风扇速度增加到高速（70% 速度）
* 在 75°C 时，风扇速度增加到全速（100% 速度）

温度下降时，使用相同的映射，并带有 5°C 的滞后；当温度下降到低于上述每个阈值 5°C 时，风扇速度会降低。

启动时，风扇被打开，并检查转速计输入以查看风扇是否在旋转。如果是，则启用 cooling_fan 设备树覆盖。默认情况下，该覆盖位于 bcm2712-rpi-5-b.dtb 中，但可以通过 status=disabled 进行更改。
...
要从树莓派 OS 内部更改启动模式或引导加载程序版本，请运行 raspi-config。

1. 升级树莓派 OS 以获取 rpi-eeprom 软件包的最新版本。
2. 运行 sudo raspi-config 。
3. 选择 Advanced Options 。
4. 选择 Bootloader Version 。
5. 选择 Default 以恢复出厂设置或选择 Latest 以获取最新的引导程序发布。
...
转到 Advanced Options ，然后选择 Bootloader Version 。选择 Latest 并选择 Yes 确认。选择 Finish 并确认要重新启动。重新启动后，再次打开命令提示符并更新您的系统：

```
$ sudo apt update
```

如果运行 sudo rpi-eeprom-update ，您会看到引导加载程序的更新版本是 latest 发布。
...
重新启动，然后运行 sudo rpi-eeprom-update 。现在您应该看到 CURRENT 日期已更新为引导程序的最新版本。
...
| NOTE | 如果设置了 FREEZEVERSION 引导加载程序配置，则更新服务将跳过任何自动更新。这样可以避免在安装了多个操作系统或更换 SD 卡时需要逐个禁用更新服务。 |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |

#### `rpi-eeprom-update`

树莓派操作系统使用 rpi-eeprom-update 脚本实现自动更新服务。该脚本还可以以交互方式运行或包装以创建定制的引导加载程序更新服务。
...
-d 标志指示 rpi-eeprom-update 使用指定的镜像文件中的配置，而不是自动迁移当前的配置。
...
固件发布状态对应于引导加载程序固件镜像的特定子目录（ /lib/firmware/raspberrypi/bootloader/... ），可以更改以选择不同的发布流。

* default -已更新以支持新硬件、关键错误修复，并通过 latest 版本测试过的新功能的定期更新。
* latest - 当新功能可用时进行更新
...
| NOTE | 您可以通过编辑 /etc/default/rpi-eeprom-update 文件并将 FIRMWARE_RELEASE_STATUS 条目更改为适当的流来更改在更新过程中使用的发布流。 |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------- |
...
在开机时，BCM2711 和 BCM2712 上的 ROM 会在 SD 卡的启动分区的根目录中查找一个名为 recovery.bin 的文件。如果找到一个有效的 recovery.bin ，那么 ROM 会执行这个文件，而不是 EEPROM 的内容。这种机制可以确保引导加载程序闪存镜像始终可以被重置为具有工厂默认设置的有效镜像。
...
* 如果引导加载程序更新镜像称为 pieeprom.upd ，则更新完成后将 recovery.bin 重命名为 recovery.000 ，然后系统重新启动。由于 recovery.bin 不再存在，ROM 将从 SPI 闪存加载新更新的引导加载程序，然后正常启动操作系统。
* 如果引导加载程序更新镜像称为 pieeprom.bin ，那么更新完成后 recovery.bin 将停止。成功后，HDMI 输出将会是绿色，绿色活动 LED 会快速闪烁。如果更新失败，HDMI 输出将会是红色，并且通过活动 LED 显示错误代码。
* .sig 文件包含相应镜像文件的十六进制 sha256 校验和；将来可能会添加其他字段。
* 在 BCM2711 和 BCM2712 上找到的 ROM 不支持从 USB 大容量存储设备或 TFTP 加载 recovery.bin 。相反，引导加载程序的更新版本支持自更新机制，使引导加载程序能够重新刷新 SPI 闪存本身。请参阅引导加载程序配置页面上的 ENABLE_SELF_UPDATE 。
* 临时 EEPROM 更新文件在启动时由 rpi-eeprom-update 服务自动删除。
...
从树莓派 4 引导加载程序的 2020-04-16 版本开始，诊断信息可以在 HDMI 显示器上显示。要查看此诊断信息，请关闭树莓派 4，移除 SD 卡，然后重新启动。类似以下的诊断显示应该出现在连接的显示器上。
...
该诊断页面也会出现在引导加载程序无法从插入的 SD 卡引导，或无法进行网络引导的情况下；例如，如果卡上没有可引导的映像，或者如果卡有缺陷，或者网络引导参数不正确。

一旦显示诊断页面，只能通过重新插拔设备电源（即拔掉然后重新插上电源）来重新启动。

顶部一行描述了 Raspberry Pi 型号及其内存容量。QR 码是一个指向下载页面的链接。
...
| `secure-boot`   | 如果启用了安全启动，则显示处理器版本（B0/C0）和签名启动状态标志。否则，此行为空。                                       |
...
| NOTE | 这仅用于诊断引导失败；它不是一个交互式引导程序。如果您需要交互式引导程序，请考虑使用类似 U-Boot 的工具。 |
| ------ | ---------------------------------------------------------------------------------------------------------- |
...
基于 BCM2837 的 Raspberry Pis 可以执行 USB 主机和以太网引导 - 也就是说，Raspberry Pi 2B 版本 1.2，Raspberry Pi 3B 和 Raspberry Pi 3B+（由于 Raspberry Pi 3A+没有内置以太网接口，无法进行网络引导）。此外，所有早于 Raspberry Pi 4 的 Raspberry Pi 型号可以使用仅 bootcode.bin 方法启用 USB 主机引导。

| NOTE | 自 Raspberry Pi 4 以来，旗舰设备不再使用 bootcode.bin 文件。相反，这些设备使用位于板载 EEPROM 芯片中的引导加载程序。有关更多信息，请参阅 EEPROM 引导流程和 SPI 引导 EEPROM 的文档。 |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

格式化 SD 卡为 FAT32 并复制最新的 bootcode.bin 。SD 卡必须插入树莓派中才能启动。一旦 bootcode.bin 从 SD 卡加载，树莓派将继续使用 USB 主机模式启动。

这对于基于 BCM2835 和 BCM2836 芯片的树莓派 1、2 和 Zero 型号非常有用，在树莓派 3 无法启动的情况下（与烧录到 BCM2837A0 中的启动代码相比，最新的 bootcode.bin 为树莓派 3B 提供了额外的错误修复）。

如果您对于质量存储设备仍然无法工作有问题，即使使用了这个 bootcode.bin ，请在 SD 卡上添加一个名为“timeout”的新文件。这将使其等待质量存储设备初始化的时间延长到六秒。

### bootcode.bin UART 启用

| NOTE | 对于发布在 Raspberry Pi 4 之前的主板。 |
| ------ | ---------------------------------------- |

有关使用 EEPROM 引导加载程序启用 UART 的信息，请参阅引导加载程序配置文档。

可以启用早期阶段的 UART 来调试引导问题（与上述 bootcode.bin 仅引导模式非常有用）。要做到这一点，请确保您的固件版本较新（包括 bootcode.bin ）。要检查当前固件是否支持 UART：
...
接下来，将合适的 USB 串行电缆连接到主机计算机（Raspberry Pi 将适用，尽管您可能会发现，最简单的方法是使用 USB 串行电缆，因为它可以在不需要烦人的 config.txt 设置的情况下直接使用）。在 Raspberry Pi 或计算模块上使用标准的引脚 6、8 和 10（GND、GPIO14、GPIO15）。

然后在 Linux 或 macOS 上使用 screen ，或者在 Windows 上使用 putty 来连接到串口。

设置串口接收速率为 115200-8-N-1，然后启动您的 Raspberry Pi。您应该立即从设备上获得 bootcode.bin 的串行输出。
...
固件会根据 OTP 位在引导时选择两种模式中的一种。两位控制 USB 引导。第一位启用 USB 设备引导，默认情况下已启用；第二位启用 USB 主机引导。

如果设置了 USB 主机引导模式位，则处理器会读取 OTGID 引脚来决定是作为主机引导（驱动到零，就像在任何树莓派 Model B/B+ 上一样）还是作为设备引导（保持漂浮）。树莓派 Zero 通过 USB 连接器可以访问 OTGID 引脚；计算模块可以通过边缘连接器访问 OTGID 引脚。

其他一些 OTP 位可允许某些 GPIO 引脚选择引导模式。

### U 盘设备引导模式

| NOTE | U 盘引导可用于旗舰系列的 Compute Module 系列，Zero 系列和 Model A 变种。 |
| ------ | -------------------------------------------------------------------------- |

当启用此引导模式（通常在无法从 SD 卡引导后），Raspberry Pi 将其 USB 设备模式，等待主机发送的 USB 重置。在 Github 上可以找到示例代码，展示主机需如何与 Raspberry Pi 进行通信。
...
| NOTE | 自树莓派计算模组 3、零系列 2 W 版、树莓派 2B（1.2 版）、树莓派 3B 以及自树莓派 3B+以来，主机引导功能已经在 Compute Module 系列和所有旗舰系列设备上提供。树莓派 3A+支持大容量存储引导，但不支持网络引导。 |
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

USB 主机引导模式采用以下逻辑:

1. 启用 USB port 并等待 D+ 线拉高，表示连接了 USB 2.0 设备（我们只支持 USB2.0）
2. 如果设备是一个集线器：

    1. 启用所有集线器的下游 ports 的电源
    2. 对于每个port，循环最多两秒（如果 program_usb_boot_timeout=1 已设置，则为五秒）
...
在树莓派 3B，3A+和 3B+上，默认禁用主机引导。要启用 USB 主机引导，将包含 program_usb_boot_mode=1 的一行添加到 /boot/firmware/config.txt 的末尾。

| WARNING | 对 OTP 进行的任何更改都是永久性的，无法撤销。<br /><br />在 Raspberry Pi 3A+ 上，将 OTP 位设置为启用 USB 主机引导模式将永久性地阻止该 Raspberry Pi 以 USB 设备模式启动。 |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |

## USB 大容量存储引导

| NOTE | 自 Compute Module 3 起，Zero 2 W 起的 Zero 系列，以及自 Raspberry Pi 2B（版本 1.2）起的所有旗舰系列设备均可使用。 |
| ------ | ------------------------------------------------------------------------------------------------------------------- |

USB 大容量存储启动允许您从 USB 大容量存储设备（如闪存驱动器或 USB 磁盘）启动 Raspberry Pi。连接 USB 设备时，特别是硬盘和 SSD，需要注意它们的功耗要求。连接多个磁盘通常需要额外的外部电源，可以是有电源的磁盘外壳或有电源的 USB 集线器。

| NOTE | Raspberry Pi 4B 之前的型号存在已知问题，可能导致无法使用某些 USB 设备进行启动。 |
| ------ | --------------------------------------------------------------------------------- |

### 具有 EEPROM 引导加载程序的设备

Raspberry Pi 4 和更新的旗舰系列设备以及自计算模块 4 和 4S 起，支持默认情况下的 USB 引导，只要在 BOOT_ORDER 配置中指定 USB 引导。

| NOTE | 早期版本的 Raspberry Pi 4 可能需要更新引导加载程序才能从 USB 启动。 |
| ------ | --------------------------------------------------------------------- |

| NOTE | Compute Module 4 的早期版本可能需要更新引导加载程序才能从 USB 启动。 |
| ------ | ---------------------------------------------------------------------- |
...
在树莓派 2B v1.2，3A+，3B，Zero 2 W 和 Compute Module 3 和 3+ 上，必须先启用 USB 主机启动模式。这允许 USB 大容量存储启动和网络启动。

| NOTE | 树莓派 3A+ 和 Zero 2 W 不支持网络启动。 |
| ------ | ----------------------------------------- |

要启用这些设备的 USB 主机启动模式，请在 OTP（一次可编程）内存中设置 USB 主机位。要设置该位，请从包含以下行的 SD 卡引导 /boot/firmware/config.txt 。设置该位后，您可以在没有 SD 卡的情况下从 USB 启动。

#### 使用 OTP 启用 USB 主机启动模式

| WARNING | 对 OTP（一次可编程）内存所做的任何更改都是永久性的，无法撤消。<br /><br />在 Raspberry Pi 3A+ 上，设置 OTP 位以启用 USB 主机启动模式将永久阻止该 Raspberry Pi 进入 USB 设备模式启动。 |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

使用任何安装了 Raspberry Pi OS 的 SD 卡来编程 OTP 位。
...
然后使用 sudo reboot 重新启动您的 Raspberry Pi。要检查 OTP 是否已正确编程，请运行以下命令：
...
过程与 SD 卡相同 - 使用操作系统镜像刷新 U 盘存储设备。

准备存储设备后，连接驱动器并启动 Raspberry Pi，注意外部驱动器的额外 USB 电源需求。

大约五到十秒后，Raspberry Pi 应该开始启动，并在连接的显示器上显示彩虹闪屏。请确保在 Raspberry Pi 上没有插入 SD 卡，因为如果插入了 SD 卡，它将首先从 SD 卡启动。

查看引导模式文档，了解引导顺序和备选引导模式（网络、USB 设备、GPIO 或 SD 卡启动）。

### 已知问题

* 检查可引导 USB 设备的默认超时时间为两秒。一些闪存驱动器和硬盘启动速度太慢。可以将此超时时间延长到五秒（在 SD 卡中添加一个新文件 timeout ），但要注意一些设备可能需要更长时间来响应。
* 一些闪存驱动器具有非常特定的协议要求，这不受引导码处理，因此可能不兼容。

### 特殊 bootcode.bin -仅引导模式

在 Raspberry Pi 2B v1.2、3A+、3B 和 3B+ 上，如果您无法使用特定的 USB 设备来启动您的 Raspberry Pi，您可以使用 bootcode.bin 专用启动模式。Raspberry Pi 仍会从 SD 卡启动，但只会从 SD 卡中读取 bootcode.bin ；您的操作系统的其余部分存储在 USB 设备上。

### 硬件兼容性

在从 USB 大容量存储设备启动之前，请验证该设备在 Linux 下正常工作。使用 SD 卡引导，插入 USB 大容量存储设备。这应该显示为可移动驱动器。这在使用 USB SATA 适配器时尤其重要，该适配器可能由引导加载程序支持大容量存储模式，但如果 Linux 选择 USB 附加 SCSI-UAS 模式，则可能失败。

硬盘驱动器（HDD）通常需要一个有电源的 USB 集线器。即使一切看起来正常，没有一个有电源的 USB 集线器，您可能会遇到间歇性故障。

### 多个可引导驱动器

在搜索可引导分区时，引导加载程序同时扫描所有 USB 大容量存储设备，并选择首个响应的设备。如果引导分区不包含适当的 start.elf 文件，则引导加载程序将尝试下一个可用设备。没有根据 USB 拓扑结构指定引导设备的方法；这会减慢引导速度并增加不必要的配置复杂性。

| NOTE | 使用 config.txt 文件条件过滤器来选择复杂设备配置中的备用固件。 |
| ------ | ---------------------------------------------------------------- |
...
在树莓派 4 和树莓派 5 上，网络引导是在 EEPROM 中的第二阶段引导加载程序中实现的。有关更多信息，请参阅树莓派 4 引导加载程序配置。
...
* TFTP RRQ `bootcode.bin`
  ...
  * 服务器会回复包含文件头部块编号的数据块（512 字节）中的第一个块，文件已存在

    * 树莓派回复 TFTP ACK 数据包，包含块编号，并重复直到最后一个非 512 字节块
* TFTP RRQ bootsig.bin 下载请求

  * 这通常会导致一个错误 file not found 。这是预料之中的，TFTP 引导服务器应该能够处理它。

从这一点开始， bootcode.bin 代码继续加载系统。它将尝试访问的第一个文件是 <serial_number>/start.elf 。如果这不会导致错误，那么要读取的其他文件将会在前面加上 serial_number 。这很有用，因为它使您能够为您的 Raspberry Pis 创建具有不同 start.elf /内核的独立目录。

要获取设备的序列号，您可以尝试使用这种引导模式，并查看使用 tcpdump / wireshark 访问的文件，或者您可以运行标准的 Raspberry Pi OS SD 卡和 cat /proc/cpuinfo 。

如果您将所有文件放入 TFTP 目录的根目录中，那么随后所有的文件都将从那里访问。

### 调试网络引导模式

首先要检查的是 OTP 位是否被正确编程。为此，您需要将 program_usb_boot_mode=1 添加到 config.txt ，然后重新启动（使用可以正确引导到 Raspberry Pi OS 的标准 SD 卡）。完成后，您应该可以执行以下操作：

```
$ vcgencmd otp_dump | grep 17:
```

如果第 17 行包含 3020000a ，则 OTP 已正确编程。现在，您可以拔掉 SD 卡，插入以太网，然后树莓派上电约 5 秒后以太网指示灯应该亮起。

要在服务器上捕获以太网数据包，请在 tftpboot 服务器上（或者如果它们不同，则在 DHCP 服务器上）使用 tcpdump。否则，您将无法看到发送的数据包，因为网络交换机不是集线器！

```
$ sudo tcpdump -i eth0 -w dump.pcap
```

将从 eth0 写入文件 dump.pcap 中的所有内容。然后，您可以对数据包进行后处理或上传到 cloudshark 进行通信。

#### DHCP 请求/响应

至少您应该看到一个类似以下内容的 DHCP 请求和回复：
...
Vendor-Option Option 43 包含了回复的重要部分。这个部分必须包含字符串"Raspberry Pi Boot"。由于引导 ROM 中的一个错误，您可能需要在字符串末尾添加三个空格。

#### TFTP 文件读取

当供应商选项被正确指定时，您将看到随后发送的 TFTP RRQ 数据包。RRQ 可以通过第一个数据块或错误消息来回复，表明文件未找到。在少数情况下，他们甚至会收到第一个数据包，然后由 Raspberry Pi 中止传输（当检查文件是否存在时会发生这种情况）。下面的示例仅包括三个数据包：原始读取请求、第一个数据块（始终是包含一个标头和 512 字节数据的 516 字节，尽管最后一个块始终少于 512 字节且可能为零长度），以及第三个数据包（包含与数据块中帧编号匹配的确认帧号）。
...
The Raspberry Pi will attempt a DHCP request five times with five seconds in between, for a total period of 25 seconds. If the server is not available to respond in this time, then the Raspberry Pi will drop into a low-power state. There is no workaround for this other than bootcode.bin on an SD card. 重试  错误原因

#### TFTP server on separate subnet not supported 重试  错误原因

Fixed in Raspberry Pi 3 Model B+ (BCM2837B0). 重试  错误原因
...
在 DHCP 回复中，“树莓派启动”字符串由于计算字符串长度的错误而需要额外的三个空格。
...
在树莓派 3 Model B+ 中修复；该值设置为 32 位序列号。

#### ARP 检查可能在 TFTP 事务中间无法响应

树莓派只会在初始化阶段响应 ARP 请求；一旦开始传输数据，它将无法继续响应。

已在 Raspberry Pi 3 Model B+中修复。

#### DHCP 请求/回复/确认序列未正确实现

在启动时，Raspberry Pi 广播 DHCPDISCOVER 数据包。DHCP 服务器回复一个 DHCPOFFER 数据包。然后 Raspberry Pi 继续启动而不执行 DHCPREQUEST 或等待 DHCPACK。这可能导致两个不同设备被提供相同的 IP 地址并在未正确分配给客户端的情况下使用它。

不同的 DHCP 服务器在这种情况下有不同的行为。dnsmasq（取决于设置）将散列 MAC 地址以确定 IP 地址，并 ping 该 IP 地址以确保它尚未被使用。这减少了这种情况发生的机会，因为这需要在散列中发生碰撞。

## GPIO 启动模式

| NOTE | GPIO 引导模式仅适用于树莓派 3A+、3B、3B+、计算模块 3 和 3+。 |
| ------ | -------------------------------------------------------------- |

早期的树莓派可以配置，以允许在接通电源时使用连接到 GPIO 连接器的硬件选择启动模式。这是通过在 SoC 的 OTP 存储器中设置位来完成的。一旦设置了这些位，它们会永久分配五个 GPIO，以便进行选择。一旦设置了 OTP 位，它们就无法取消设置。您应该仔细考虑是否启用此功能，因为这五个 GPIO 线将始终控制引导。尽管在树莓派引导后可以使用这些 GPIO 进行其他功能，但您必须进行设置，以便在树莓派引导时启用所需的启动模式。
...
其中 n 是您希望使用的 GPIO 的 bank。然后重新启动 Raspberry Pi 一次，以使用此设置编程 OTP。Bank 1 是 GPIO 22-26，Bank 2 是 GPIO 39-43。除非您有一个计算模块，否则必须使用 bank 1：Bank 2 上的 GPIO 仅在计算模块上可用。由于 OTP 位的排列方式，如果您首先为 Bank 1 编程 GPIO 引导模式，那么稍后可以选择 Bank 2。反之不成立：一旦选择 Bank 2 作为 GPIO 引导模式，就无法选择 Bank 1。

一旦启用了 GPIO 引导模式，树莓派将不再启动。您必须拉高至少一个引导模式 GPIO 引脚，以使树莓派启动。
...
以上表格中的 USB 选项既选择 USB 设备引导模式又选择 USB 主机引导模式。要使用 USB 引导模式，必须在 OTP 存储器中启用它。有关更多信息，请参阅 USB 设备引导和 USB 主机引导。

#### 后续的 Raspberry Pi 3B（带金属盖的 BCM2837B0）、Raspberry Pi 3A+、3B+ 和 Compute Module 3+
...
| NOTE | 各种启动模式按照 GPIO 线的数字顺序尝试，即 SD0，然后 SD1，然后 NAND 等等。 |
| ------ | ---------------------------------------------------------------------------- |

### 启动流程

SD0 是 Broadcom SD 卡/MMC 接口。当 SoC 内部的引导 ROM 运行时，它始终将 SD0 连接到内置的 microSD 卡槽。在带有 eMMC 设备的计算模块上，SD0 连接到该设备；在 Compute Module Lite 上，SD0 可用于边缘连接器，并连接到 CMIO 载板上的 microSD 卡槽。SD1 是 Arasan SD 卡/MMC 接口，也能够支持 SDIO。所有具有内置无线局域网的树莓派型号都使用 SD1 通过 SDIO 连接到无线芯片。

GPIO 线上的默认拉电阻为 50KΩ，详见 BCM2835 ARM 外围设备数据表第 102 页。推荐使用 5KΩ的拉电阻将 GPIO 线上拉高：这样可以使 GPIO 正常工作，同时不会消耗太多功率。

## NVMe 固态硬盘引导

NVMe（非易失性内存表达）是通过 PCIe 总线进行外部存储访问的标准。您可以通过 Compute Module 4（CM4）IO 板或树莓派 5 上的 PCIe 槽连接 NVMe 驱动器。通过一些额外配置，您可以从 NVMe 驱动器启动。
...
最新的树莓派 OS 发布包含您所需的一切。使用树莓派镜像制作工具将树莓派 OS 镜像安装到您的驱动器上。

### 编辑 EEPROM 启动顺序

对于 Raspberry Pi 5，您需要启动 Raspberry Pi OS 以编辑启动顺序。您可以从 SD 卡或 USB 驱动器启动 Raspberry Pi 完成此步骤。即使更改启动设备，EEPROM 配置也会持久保留，因为 EEPROM 配置存储在板上本身。

使用 Raspberry Pi 配置命令行界面来更新启动加载程序：
...
对于 CM4，请使用 rpiboot 更新引导加载程序。您可以在 USB 启动 GitHub 存储库中找到构建 rpiboot 并配置 IO 板切换 ROM 到 usbboot 模式的说明。

对于带有 eMMC 的 CM4 版本，请确保您已将 NVMe 设置为引导顺序中的第一项。请记住在 recovery/boot.conf 中将 NVMe 引导模式 6 添加到 BOOT_ORDER 中。
...
所有 HTTP 下载必须签名。The bootloader includes a public key for the files on the default host fw-download-alias1.raspberrypi.com . This key will be used to verify the network install image, unless you set HTTP_HOST and include a public key in the EEPROM. This allows you to host the Raspberry Pi network install images on your own server.

| WARNING | 使用您自己的网络安装镜像将需要您对镜像进行签名，并将您的公钥添加到 EEPROM 中。如果您然后应用公共 EEPROM 更新，则您的密钥将丢失，并且需要重新添加。 |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |

USBBOOT 具有编程公钥所需的所有工具。

使用以下命令将您的公钥添加到 EEPROM。 boot.conf 包含您的修改：

```
$ rpi-eeprom-config -c boot.conf -p mypubkey.pem -o pieeprom.upd pieeprom.original.bin
```

使用以下命令为您的 EEPROM 生成签名：

```
$ rpi-eeprom-digest -i pieeprom.upd -o pieeprom.sig
```

然后，使用以下命令使用您的私钥对网络安装镜像进行签名：

```
$ rpi-eeprom-digest -i boot.img -o boot.sig -k myprivkey.pem
```

最后，将 boot.img 和 boot.sig 放在您的网络服务器上，以使用您自己签名的网络安装镜像。

### 证书

为了安全起见，网络安装使用 HTTPS 从 Raspberry Pi 网站下载操作系统镜像。此功能使用我们在引导程序中包含的自有 CA 根证书来验证主机。

您可以将您自己的定制 CA 证书添加到设备的 EEPROM 中，以安全地从您自己的网站下载镜像。使用 --cacertder 工具的 rpi-eeprom-config 选项添加 DER 编码的证书。您必须在 EEPROM 配置设置中放置证书的哈希值，以确保证书未被修改。
...
* 将 <your_website> 替换为您的网站，例如 yourserver.org
* 将 <path_to_files> 替换为托管在您的网站上的 OS 镜像的路径，例如 path/to/files
...
当您指定 HTTP_CACERT_HASH 时，Network Install 将通过port端口上的 HTTPS 下载镜像。 没有哈希时，Network Install 将通过port端口上的 HTTP 下载镜像。
...
在网络启动期间，您的 Raspberry Pi 应该使用 HTTPS 而不是 HTTP。要查看由网络安装解析的完整 HTTPS URL 进行下载，请检查引导输出：
...
如果启用了安全启动，则 Raspberry Pi 只能运行由客户私钥签名的代码。因此，如果您想要在安全启动的情况下使用网络安装或 HTTP 启动模式，您必须使用自己的密钥签署 boot.img 并生成 boot.sig ，并在某处托管这些文件供下载。EEPROM 中的公钥将用于验证镜像。

如果启用了安全启动且未设置 HTTP_HOST，则网络安装和 HTTP 引导将被禁用。

有关安全启动的更多信息，请参阅 USBBOOT 。

## 引导顺序

| IMPORTANT | 此启动顺序仅适用于基于 BCM2837 和 BCM2837B0 的树莓派型号。在此之前的型号上，树莓派将尝试 SD 卡引导，然后是 USB 设备模式引导。有关树莓派 4 和树莓派 5 的引导顺序，请参阅 EEPROM 引导流程部分。 |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| NOTE | 如果没有插入 SD 卡，SD 引导模式将在五秒钟后失败。为了缩短这个时间并更快地切换到 USB 模式，您可以插入一个没有任何内容的 SD 卡，或者使用上面描述的 GPIO 引导模式 OTP 设置，只启用 USB。GPIO 的默认拉取在 ARM 外围设备数据手册的第 102 页上有定义。如果引导时的值不等于默认拉取值，则启用该引导模式。USB 枚举是一种使下游设备在集线器上获取电源，然后等待设备拉动 D+和 D-线以指示其是 USB 1 还是 USB 2 的方法。这可能需要一段时间：对于某些设备，硬盘驱动器可能需要长达三秒的时间来启动并开始枚举过程。因为这是检测硬件连接的唯一方法，我们必须等待最少的时间（两秒）。如果设备在最大超时时间后仍未响应，则可以使用 program_usb_boot_timeout=1 在 config.txt 中增加超时时间至五秒。MSD 引导优先于以太网引导。不再需要第一个分区是 FAT 分区，因为 MSD 引导将继续搜索第一个分区之外的 FAT 分区。引导 ROM 现在还支持 GUID 分区，并已测试过使用 Mac、Windows 和 Linux 分区的硬盘。使用供应商 ID 0x0424 和产品 ID 0xec00 可以检测 LAN951x，这与独立的 LAN9500 设备不同，后者的产品 ID 是 0x9500 或 0x9e00 。要使用独立的 LAN9500，需要添加 I2C EEPROM 以更改这些 ID 以匹配 LAN951x。 |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
USB 设备引导模式在制造时默认启用，但 USB 主机引导模式只有在 program_usb_boot_mode=1 时才启用。一旦启用，处理器将使用处理器上的 OTGID 引脚的值来在这两种模式之间做出决定。在任何树莓派模型 B/B+ 上，OTGID 引脚被驱动为 0，因此一旦启用，只能通过主机模式引导（无法通过设备模式引导，因为 LAN951x 设备挡住了路径）。

如果将 OTGID 引脚浮置（例如，插入 PC），则 Raspberry Pi Zero 或计算模块将作为 USB 设备引导启动，因此您可以将 bootcode.bin 推入设备中。这样做的 usbboot 代码可在 GitHub 上找到。

## EEPROM 引导流程

自 Raspberry Pi 4 起，Raspberry Pi 旗舰设备使用 EEPROM 启动加载程序。与以前的产品相比，主要区别在于第二阶段启动加载程序是从 SPI 闪存 EEPROM 装载，而不是像以前的产品一样从 bootcode.bin 文件装载。
...
| NOTE | recovery.bin 是一个用于刷新引导加载程序 SPI EEPROM 映像的最小第二阶段程序。 |
| ------ | ----------------------------------------------------------------------------- |
...
#### Differences on Raspberry Pi 5 重试  错误原因

* 电源按钮用于从 PMIC STANDBY 或 HALT 唤醒，而不是 GPIO 3 。
* 固件加载 start.elf 而不是 start.elf 。实际上，引导加载程序具有嵌入版本的 start.elf 。
* 连接到 3A 电源时，默认禁用 USB 启动。在 /boot/firmware/config.txt 中设置 usb_max_current_enable=1 以启用 USB 启动。或者，您可以在 USB 启动失败时按一次电源按钮临时启用 usb_max_current_enable 并继续引导。但是，如果通过按电源按钮启用，则此设置在重新启动后不会持续。
...
要设置 tryboot 标志，请在 reboot 命令中的分区号后添加 tryboot 。通常，分区号默认为零，但如果添加了额外的参数，则必须指定。传递参数给 reboot 时始终使用引号：它只接受一个参数：

```
$ sudo reboot '0 tryboot'
```

所有树莓派型号都支持 tryboot ，但是在树莓派 4 Model B 1.0 和 1.1 版本上，EEPROM 不能写保护。这是因为旧的树莓派 4B 设备必须重置电源供应（丢失 tryboot 状态），所以这些信息存储在 EEPROM 中。
...
当从内部的 boot.img 镜像加载文件时， tryboot_a_b 属性会被隐式设置为 1 。
...
如果 1 和 WAKE_ON_GPIO=0 ，那么 sudo halt 将关闭所有 PMIC 输出。这是 halt 的最低功耗状态，但可能会导致一些 HATs 出现问题，因为 5V 仍将处于开启状态。 GLOBAL_EN 必须短接至地来启动。

树莓派 400 有一个专用的电源按钮，即使处理器关闭也能工作。这个行为默认启用，但 WAKE_ON_GPIO=2 可以设置为使用外部 GPIO 电源按钮，而不是专用电源按钮。
...
BOOT_ORDER 设置允许对不同启动模式的优先级进行灵活配置。它表示为一个 32 位无符号整数，其中每个半字节代表一个启动模式。启动模式按照从最低有效半字节到最高有效半字节的顺序尝试。
...
RPIBOOT 用于与计算模块 4 一起使用，加载自定义调试镜像（例如，Linux RAM-disk）而不是正常的引导。这应该是最后的引导选项，因为它目前不支持超时或重试。
...
为了支持每个 Raspberry Pi 的唯一 TFTP 启动目录，引导加载程序会将文件名前缀添加到特定于设备的目录。如果在带前缀的目录中找不到 start4.elf 或 start.elf，则会清除前缀。

在早期型号上，序列号被用作前缀，但在树莓派 4 和 5 上，MAC 地址不再从序列号生成，这使得通过检查 DHCPDISCOVER 包自动创建 tftpboot 目录在服务器上变得困难。为了支持这一点，TFTP_PREFIX 可能会定制为 MAC 地址、固定值或序列号（默认）。
...
在之前的版本中，客户端 GUID（Option97）只是将序列号重复四次。默认情况下，新的 GUID 格式为四字符码（FourCC）的串联（ RPi4 0x34695052 适用于 Raspberry Pi 4 或 RPi5 0x35695052 适用于 Raspberry Pi 5），板子版本（例如 0x00c03111 或 0x00d04170 ）（4 字节），MAC 地址的最低有效 4 字节以及 4 字节的序列号。这旨在是独一无二的，同时也向 DHCP 服务器提供结构化信息，使得可以识别 Raspberry Pi 4 和 5 计算机，而无需依赖以太网 MAC OUID。

指定 DHCP_OPTION97=0 以恢复旧行为，或指定非零十六进制值以指定自定义的 4 字节前缀。
...
* 2022 年之前，SD 启动中未启用自更新功能。在树莓派 4 上，ROM 已经可以从 SD 卡加载 recovery.bin。在 CM4 上，自更新和 recovery.bin 都没有任何效果，需要使用 USB 启动（请参阅计算模块 EEPROM 引导程序文档）。
...
以前，此属性仅由 rpi-eeprom-update 脚本检查。但是，现在自更新已启用，引导加载程序还将检查此属性。如果设置为 1，则将覆盖 ENABLE_SELF_UPDATE 以停止自动更新。要禁用 FREEZE_VERSION ，您必须使用带有 recovery.bin 的 SD 卡引导。
...
嵌入式 Raspberry Pi 制造者应用程序会在启动时下载一个 JSON 文件进行配置。

您可以更改内嵌的 Raspberry Pi 制作程序应用程序使用的 JSON 文件的网址，以便提供您自己的映像。您可以通过 --repo 参数传递网址来测试标准的 Raspberry Pi 制作程序应用程序。
...
NETCONSOLE 复制调试消息到网络接口。IP 地址和 ports 由 NETCONSOLE 字符串定义。

| NOTE | NETCONSOLE 阻塞，直到以太网连接建立或超时发生。超时值为 DHCP_TIMEOUT ，尽管除非请求网络引导，否则不会尝试 DHCP。 |
| ------ | ------------------------------------------------------------------------------------------------------------------ |
...
当树莓派重新启动时，硬件会关闭 USB 电源。短暂的断电时间可能会导致某些 USB 设备出现问题，因此可以使用此参数强制执行较长的断电时间，就像电缆被物理拔掉一样。

在 RaspberryPi 4 版本 1.3 及更早版本上，可配置/长关机需要启用 XHCI 控制器，因此实际上会先进行短关机，然后是较长的可配置关机。通过将此参数设置为零，可以跳过较长的可配置关机。
...
* 嵌入式 VL805 固件假定与 Raspberry Pi 4B 相同的 USB 配置（两个 USB 3.0 ports 和四个 USB 2.0 ports）。不支持加载替代的 VL805 固件镜像，应该改用专用的 VL805 SPI EEPROM 来替换这样的配置。
...
| WARNING | 如果您配置引导加载程序时出现无法引导的无效配置，则必须重新刷新引导加载程序 EEPROM 为有效配置以引导。 |
| --------- | ------------------------------------------------------------------------------------------------------ |

| TIP | 一些配置属性存储在 config.txt 中。有关这些属性的更多信息，请参阅配置属性。 |
| ----- | ---------------------------------------------------------------------------- |
...
40 针通用输入/输出（GPIO）连接器可用于将显示并行接口（DPI）显示器连接到树莓派设备，作为使用专用显示串行接口（DSI）或高清多媒体接口（HDMI）的替代方案ports。
...
| NOTE | DPI 输出引脚上的颜色值可以以 565、666 或 24 位模式显示（请参阅下表和参数 output_format 的 dpi_output_format 部分）的多种方式： |
| ------ | -------------------------------------------------------------------------------------------------------------------------------- |
...
必须禁用使用冲突 GPIO 引脚的所有其他外设叠加层。在 config.txt 中，请注意注释或反转启用 I2C 或 SPI 的任何 dtparams：
...
自动检测允许您的树莓派与显示器连接，无需手动配置设备树叠加。默认情况下启用了自动检测。您可以通过将以下行添加到 config.txt 来启用显示器自动检测：

```
display_auto_detect=1
```

用 0 替换 1 以禁用自动检测。当您连接启用自动检测的官方树莓派显示器时，KMS 会自动确定显示模型并配置适当的显示设置。

#### 手动配置显示器

| NOTE | 在树莓派 OS Bookworm 或更高版本中，先前用于设置 DPI 的 dpi_output_format 和 dpi_timings 条目已被 vc4-kms-dpi-generic 叠加层取代。 |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------- |

要使用除官方 Raspberry Pi 显示屏之外的任何显示屏，必须在 BSD 中指定一个 dtoverlay 条目。 面板制造商应在 Linux 内核代码中配置您的显示屏的定时，并提供一个叠加层来启用这些设置。 请参阅 Adafruit Kippah 显示屏条目以获取示例。 以下示例演示如何在您的 FreeBSD 文件中为 Kippah 显示屏设置一个 dtoverlay 条目：

```
dtoverlay=vc4-kms-kippah-7inch-overlay
```

显示定时通常在内核中定义，但您也可以在提供的 BSD 驱动程序中定义它们。 如果您的面板在内核代码中缺乏定义的叠加层，则可以使用 BSD 驱动程序将显示定时定义为参数。 这使您能够为任何显示屏手动配置设备树条目。
...
| NOTE | 设备树行长度不得超过 80 个字符。当设置需要超过 80 个字符的行时，请将该参数的赋值拆分为多行。 |
| ------ | ---------------------------------------------------------------------------------------------- |
...
您可以在所有当前的 Raspberry Pi 主板上找到一个 40 针 GPIO（通用输入/输出）排针。所有主板上的 GPIO 排针均采用 0.1 英寸（2.54 毫米）的间距。

| NOTE | 零和 Pico 设备上没有带有“H”后缀的排针。 |
| ------ | ------------------------------------------- |
...
可以通过打开终端窗口并运行命令 pinout 在树莓派上访问 GPIO 参考。此工具由 GPIO Zero Python 库提供，在树莓派 OS 中默认安装。

| WARNING | 尽管连接简单的组件到 GPIO 引脚是安全的，但要小心如何连接线路。LED 应该有电阻来限制通过它们的电流。不要为 3.3V 组件使用 5V。不要直接将电机连接到 GPIO 引脚，而是使用 H 桥电路或电机控制板。 |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

### 权限

为了使用 GPIO ports，您的用户必须是 gpio 组的成员。默认用户帐户默认是成员，但您必须使用以下命令手动添加其他用户：
...
BCM2835 封装上的 GPIO 连接有时在外设数据表中被称为“pads”，这是一个半导体设计术语，意思是“芯片连接到外部世界”。
...
几乎所有的 GPIO 引脚都具有替代功能。 SoC 内部的外设模块可以选择出现在一组 GPIO 引脚中的一个或多个上，例如，I2C 总线可以配置为至少三个独立位置之一。当引脚配置为替代功能时，例如，控制驱动强度或施密特触发器等仍然适用。

一些功能适用于所有引脚，另一些适用于特定引脚：
...
板上有两个 5V 针脚和两个 3.3V 针脚，以及一些接地针脚（GND），这些不能重新配置。剩余的针脚都是通用的 3.3V 针脚，意味着输出被设置为 3.3V，输入是 3.3V 兼容的。
...
| V\~OH\~ | 输出高电压^b^ | IOH = 2 毫安    | 3.0  | -    | -    | V      |
| I\~OL\~ | 输出低电流^c^ | 电压 = 0.4 伏特 | 18   | -    | -    | 毫安   |
...
下表显示了 BCM2711 系列产品（例如 Raspberry Pi 4 和 Raspberry Pi 400）上 GPIO 引脚的电压规格。有关计算模块的信息，请参阅相关数据表。
...
GPIO 驱动强度并不表示最大电流，而表示在此电流下 pad 仍将符合规范。您应该设置 GPIO 驱动强度以匹配所连接的设备，以便设备能够正常工作。

### 控制驱动强度

在 pad 内部有多个并联驱动器。如果将驱动强度设置为低（ 0b000 ），则大多数驱动器将处于三态状态，因此不会对输出电流产生影响。如果增加驱动强度，则会并联更多驱动器。图表显示了该行为。

| WARNING | 对于 Raspberry Pi 4、Raspberry Pi 400 和 Compute Module 4，当前级别是图表中显示数值的一半。 |
| --------- | --------------------------------------------------------------------------------------------- |
...
* 如果设置为高电平，垫片将尝试将输出驱动到电源电压（3.3V）
* 如果设置较低，该垫将尝试将输出引至地线（0V）

该垫将尝试将输出引至高电平或低电平。成功取决于所连接设备的要求。如果该垫短接至地线，则无法输出高电平。它将尽可能输送尽可能多的电流，电流仅受内部电阻限制。

如果该垫输出高电平且短接至地线，在一段时间后将损坏。如果您将其连接至 3.3V 并输出低电平同样也是如此。

符合规格是由保证的电压水平确定的。由于引脚是数字的，因此有两个电压级别，高电平和低电平。I/O ports 有两个处理输出电平的参数：
...
例如，驱动强度为 16mA 表示如果您将端口设置为高电平，可以吸收高达 16mA 电流，并且输出电压保证 >=V~OH~。这也意味着，如果您将驱动强度设置为 2mA，但吸收了 16mA 电流，则电压将不会达到 V~OH~，而会更低。事实上，可能不足以被外部设备识别为高电平。

GPIO 引脚的物理特性有更多信息。

| NOTE | 在计算模块设备上，可以将 VDD IO 从标准的 3.3V 更改。在这种情况下，V~OL~和 V~OH~将根据 GPIO 部分的表格进行更改。 |
| ------ | ----------------------------------------------------------------------------------------------------------------- |

Raspberry Pi 的 3.3V 电源设计每个 GPIO 引脚的最大电流为~3mA。如果您将每个引脚的负载设定为 16mA，则总电流为 272mA。在此负载水平下，3.3V 电源将崩溃。会发生大电流脉冲，特别是如果有电容负载。脉冲将在附近的其他引脚之间反弹。这可能会导致与 SD 卡甚至 SDRAM 行为的干扰。

### 安全电流

所有的键盘的电子元件均设计为 16mA。这是一个安全值，在此范围内您不会损坏设备。即使您将驱动强度设置为 2mA，然后负载为 16mA，也不会损坏设备。除此之外，没有保证的最大安全电流。
...
当心同时切换输出（SSO）限制，这些限制取决于设备以及 PCB 的质量和布局，去耦电容器的数量和质量，垫子上的负载类型（电阻，电容）以及树莓派无法控制的其他因素。
...
树莓派经常被用作另一种产品的一部分。本文档描述了一些额外的功能，可用于利用您的树莓派的其他功能。
...
所有 Raspberry Pi 单板计算机（SBCs）都有内置的一次编程（OTP）存储器区域，实际上是主片上的一部分。正如其名称暗示的那样，OTP 存储器只能被写入一次（即二进制 0 只能改为 1）。一旦某位被改为 1，就永远无法恢复为 0。看待 OTP 的一种方式是将每一位视为保险丝。对其进行编程涉及有意地烧断保险丝 —— 一种不可逆的过程，因为你无法进入芯片内部去替换它！

本白皮书假定 Raspberry Pi 正在运行 Raspberry Pi 操作系统（OS），并且已完全更新最新固件和内核。
...
| NOTE | 在 BCM2712 设备上，这些数字是不同的。第 31 行是序列号，第 32 行是板子修订号码。客户行是 77 到 84（包括 77 和 84）。 |
| ------ | --------------------------------------------------------------------------------------------------------------------- |

一些这些行可以使用 vcmailbox 进行编程。这是一个 Linux 驱动程序界面，用于处理行的编程。要执行此操作，请参考文档和 vcmailbox 示例应用程序。

vcmailbox 应用程序可以直接在树莓派 OS 的命令行上使用。一个示例用法是：
...
以上使用邮箱属性接口 GET_BOARD_SERIAL ，请求大小为 8 字节，响应大小为 8 字节（发送两个整数作为请求 0,0）。对此的响应将是两个整数（0x00000020 和 0x80000000），后跟标签代码、请求长度、响应长度（第 31 位设为 1 表示它是响应），然后是 64 位序列号（其中最高 32 位始终为 0）。

### 编写和读取客户 OTP 值

| WARNING | OTP 值是一次性可编程的。一旦将位从 0 更改为 1，则无法改回。 |
| --------- | ------------------------------------------------------------- |

要设置定制 OTP 值，您需要使用 SET_CUSTOMER_OTP (0x38021) 标签如下：
...
因此，要将 OTP 客户行 4、5 和 6 编程为分别为 0x11111111、0x22222222、0x33333333，您将使用：
...
一旦锁定，客户 OTP 值将不再更改。请注意，此锁定操作是不可逆的。
...
要设置客户端的 MAC 地址，必须按正确顺序将其发送为两个 32 字的单词。您可以运行一个命令来检查它是否格式正确：

```
$ vcmailbox 0x00030085 6 6 0x44332211 0x6655
```

检查日志以查看 MAC 地址是否符合您的预期：
...
* 设备专用密钥行只能通过 vcmailbox 命令读取，该命令需要访问 /dev/vcio ，该访问权限仅限于 Raspberry Pi OS 上的 video 组。
* 树莓派计算机没有硬件保护密钥存储。建议将此功能与安全启动结合使用，以限制对这些数据的访问。
* Raspberry Pi OS 不支持加密根文件系统。
...
| NOTE | usbboot 库包含您需要的所有工具，包括 rpi-eeprom 作为 Git 子模块。 |
| ------ | ------------------------------------------------------------------- |
...
此列表包含关于寄存器的公开信息。如果某个寄存器或位未在此定义，则表示它不是公开信息。
...
| NOTE | 在 BCM2711 上，引导模式由引导加载程序 EEPROM 配置而不是 OTP 定义。 |
| ------ | -------------------------------------------------------------------- |
...
33 板属性 - 其含义取决于板模型。这可通过设备树在 /proc/device-tree/chosen/rpi-boardrev-ext 中获取
...
树莓派 5 版在板的右侧有一个 FPC 连接器。此连接器可提供 PCIe Gen 2.0 ×1 接口，用于连接快速外设。

将 PCIe HAT+ 设备连接到树莓派。您的树莓派应该会自动检测到该设备。要连接非 HAT+ 设备，请将其连接到树莓派，然后手动启用 PCIe。

有关 PCIe FPC 连接器引脚分配和创建第三方设备、配件和 HAT 所需的其他详细信息，请参阅树莓派 PCIe 标准文档。建议同时阅读树莓派 HAT+ 规格说明。

| NOTE | 目前不支持架设在交换机后面的 PCIe 设备的枚举。 |
| ------ | ------------------------------------------------ |

### 启用 PCIe

默认情况下，除非连接到 HAT+ 设备，否则不会启用 PCIe 连接器。要启用连接器，请将以下行添加到 /boot/firmware/config.txt ：
...
| WARNING | 树莓派 5 未获得 3.0 代速度认证。 PCIe 3.0 代连接可能不稳定。 |
| --------- | -------------------------------------------------------------- |

#### 通过 config.txt

连接已被证明符合 Gen 2.0 速度（5 GT/sec），但您可以强制为 Gen 3.0（10 GT/sec）速度。要启用 PCIe Gen 3.0 速度，请向 /boot/firmware/config.txt 添加以下行：

```
dtparam=pciex1_gen=3
```

重新启动您的 Raspberry Pi，并使用 sudo reboot 使这些设置生效。
...
| NOTE | 本节仅适用于带有电源按钮的 Raspberry Pi 型号，如 Raspberry Pi 5。 |
| ------ | ------------------------------------------------------------------- |

当您第一次将树莓派插入电源时，它将自动打开并启动到操作系统，无需按下按钮。

如果您运行树莓派桌面版，您可以通过简短地按下电源按钮来发起干净的关机。 一个窗口将出现询问您是否要关机、重启或注销。

选择一个选项或再次按下电源按钮来发起干净的关机。

| NOTE | 如果你运行树莓派桌面版，则可以快速连续按两次电源按钮来关闭计算机。如果你运行没有桌面界面的树莓派操作系统精简版，则按一次电源按钮来启动关机流程。 |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |

### 重新启动

如果树莓派主板已关闭但仍连接电源，按下电源按钮将重新启动主板。

| NOTE | 重置电源管理集成电路（PMIC）也可以重新启动板子。连接 HAT 可以重置 PMIC。在连接 HAT 之前，始终断开设备与电源的连接。 |
| ------ | --------------------------------------------------------------------------------------------------------------------- |
...
J2 跳线帽位于 RTC 电池连接器和板边之间。此引出使您可以向 Raspberry Pi 5 添加自己的电源按钮，方法是添加一个常开(NO)瞬时开关，将两个焊盘连接起来。简短地闭合此开关将执行与板载电源按钮相同的操作。

## 电源供应

Raspberry Pi 型号的电源供应要求有所不同。所有型号都需要 5.1V 供应，但通常根据型号增加所需电流。到 Raspberry Pi 3 为止的所有型号需要一个 micro USB 电源连接器，而 Raspberry Pi 4、Raspberry Pi 400 和 Raspberry Pi 5 使用 USB-C 连接器。

每个 Raspberry Pi 消耗的电流取决于连接的外围设备。

### 推荐的电源供应器

对于 Raspberry Pi 1、Raspberry Pi 2 和 Raspberry Pi 3，我们推荐使用 2.5A 微型 USB 供应器。对于 Raspberry Pi 4 和 Raspberry Pi 400，我们推荐使用 3A USB-C 供应器。对于 Raspberry Pi 5，我们推荐使用 27W USB-C 电源供应器。

| NOTE | 没有 Raspberry Pi 型号支持 USB-PPS。 |
| ------ | -------------------------------------- |

| NOTE | 如果您使用第三方 USB-PD 多port电源适配器，在连接树莓派时插入其他设备会导致电源适配器与树莓派之间重新协商。如果树莓派已供电，这将无缝进行。如果树莓派已关机，则该协商可能导致树莓派启动。 |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
所有具有 PoE 兼容以太网插孔的树莓派型号都需要通过以太网port绘制电源的 HAT。对于支持 PoE 的型号，我们建议使用 PoE HAT。对于支持 PoE + 的型号，我们建议使用 PoE + HAT。

### 典型功率要求

| 产品               | 推荐的 PSU 电流容量 | 最大总 USB 外设电流吸收                        | 典型裸板活动电流消耗 |
| -------------------- | --------------------- | ------------------------------------------------ | ---------------------- |
| 树莓派 1 型 A 款   | 700 毫安            | 500 毫安                                       | 200 毫安             |
| 树莓派 1 型 B 型号 | 1.2A                | 500 毫安                                       | 500 毫安             |
| 树莓派 1 型 A+     | 700 毫安            | 500 毫安                                       | 180 毫安             |
| 树莓派 1 型 B+     | 1.8A                | 1.2A                                           | 330 毫安             |
| 树莓派 2 型 B      | 1.8A                | 1.2A                                           | 350 毫安             |
| 树莓派 3 型号 B    | 2.5A                | 1.2A                                           | 400 毫安             |
| 树莓派 3 型号 A+   | 2.5A                | 仅受电源、主板和连接器评级限制。               | 350 毫安             |
| 树莓派 3 型 B+     | 2.5A                | 1.2A                                           | 500 毫安             |
| 树莓派 4 型号 B    | 3.0A                | 1.2A                                           | 600 毫安             |
| 树莓派 5           | 5.0A                | 1.6 安培（如果使用 3 安培电源则为 600 毫安培） | 800 毫安培           |
| 树莓派 400         | 3.0A                | 1.2A                                           | 800 毫安             |
| 树莓派 Zero        | 1.2A                | 仅受电源、板卡和连接器评级限制                 | 100 毫安             |
| 树莓派 Zero W      | 1.2A                | 受电源、主板和连接器额定值的限制。             | 150 毫安             |
| 树莓派 Zero 2 W    | 2A                  | 仅受电源、主板和连接器额定值限制。             | 350mA                |

| NOTE | 当连接到 5A，+5V（25W）的电源能力的适配器时，树莓派 5 为下游 USB 外设提供 1.6A 的电源。当连接到任何其他兼容的电源时，树莓派 5 将限制下游 USB 设备的电源为 600mA。 |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

大多数 Raspberry Pi 提供足够的电流给 USB 外设，以供应大多数 USB 设备，包括键盘、鼠标和适配器。然而，一些设备需要额外的电流，包括调制解调器、外部磁盘和高功率天线。要连接一个 USB 设备，其电力需求超过上表中指定的数值，请使用外部供电的 USB 集线器。

随着您在 Raspberry Pi 上使用各种接口，其电力需求会增加。GPIO 引脚总共可安全绘制 50mA；每个引脚最多可单独绘制 16mA。HDMI 使用 50mA。相机模块需要 250mA。USB 键盘和鼠标可消耗从 100mA 到 1000mA 不等的电流。请检查您计划连接到 Raspberry Pi 的设备的功率评级，并相应购买适配器。如果不确定，请使用外部供电的 USB 集线器。
...
以下表格描述了不同树莓派型号在各种工作负载期间所绘制的功率（安培）量：
...
| NOTE | 这些测量使用了标准的 Raspberry Pi OS 镜像（截至 2016 年 2 月 26 日或 2019 年 6 月对于 Raspberry Pi 4），在室温下进行，Raspberry Pi 连接到 HDMI 显示器，USB 键盘和 USB 鼠标。Raspberry Pi 3 Model B 连接到无线局域网接入点，Raspberry Pi 4 连接到以太网。所有这些功耗测量均为近似值，并未考虑来自额外 USB 设备的功耗；如果连接了多个额外 USB 设备或 HAT，功耗很容易超过这些测量值。 |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
默认情况下，关闭树莓派 5 时会消耗约 1 瓦至 1.4 瓦的电力。可以通过手动编辑 EEPROM 配置来减少这种功率消耗，使用 sudo rpi-eeprom-config -e 。将设置更改为以下内容：
...
在关机时，功耗应该降低到约 0.01 瓦特。

### 电源供应警告

从 Raspberry Pi B+（2014 年）以来的所有型号，除了 Zero 系列，都配备了低电压检测电路，可检测到如果供电电压低于 4.63 伏特（±5%），将在内核日志中添加一条条目。

如果您看到警告，请切换到更高质量的电源供应和电缆。低质量的电源会损坏存储或导致树莓派内发生不可预测的行为。

电压可能因多种原因而下降。您可能已将太多高需求的 USB 设备插入。电源供应可能不足。或者电源供应电缆可能使用太细的导线。
...
树莓派设备经常用作数据存储和监控设备，通常在可能发生突然断电的地方使用。与任何计算设备一样，电力中断可能导致存储损坏。

本白皮书提供了一些选项，介绍如何通过选择适当的文件系统和设置来防止在这些以及其他情况下的数据损坏，以确保数据完整性。

### 电源和 Raspberry Pi OS

引导加载程序通过设备树 /proc/device-tree/chosen/power 传递有关电源供应的信息。用户通常不会直接阅读此信息。

max_current max 电流（单位：毫安）
...
您无法看到 USB 电流或其他直接连接到 5V 的设备，因为这将绕过 PMIC。 您不应该期望这些值加起来等于电源供应的瓦特数。 但是，监视核心电压等事项可能很有用。

### 给背部供电

USB 规范要求 USB 设备不能向上游设备供电。如果 USB 设备向上游设备供电，则称为背部供电。通常是当连接了一个制作不良的带电 USB 集线器时会发生这种情况，并且会导致带电 USB 集线器向主机的 Raspberry Pi 供电。这不建议，因为通过集线器向 Raspberry Pi 供电将绕过内置在 Raspberry Pi 中的保护电路，使其在发生电涌时容易受到损坏。

## 实时时钟（RTC）

树莓派 5 包含一个 RTC 模块。这可以通过位于 USB-C 电源连接器右侧的主板上的 J5（BAT）连接器进行电池供电。
...
您可以设置唤醒闹钟，将主板切换到非常低功耗状态（约 3mA）。达到闹钟时间时，主板将重新上电。这对像延时图像这样的周期性任务非常有用。
...
将会使板进入非常低功耗状态，然后在 10 分钟后唤醒并重新启动。
...
| NOTE | 即使未连接到 J5 连接器的备用电池，RTC 仍然可用。 |
| ------ | -------------------------------------------------- |
...
官方电池部件是一款可充电的锂锰硬币电池，配有预装的两针 JST-SH 插头和粘性安装垫。这适用于在板子的主电源断开时为 RTC 供电。由于关机时的电流吸收为个位数µA，保持时间为几个月。

| NOTE | 不建议使用主要（非可充电）锂电池作为 RTC 的备用电源。RTC 的备用电流消耗高于大多数专用 RTC 模块，将导致短暂的使用寿命。 |
| ------ | ------------------------------------------------------------------------------------------------------------------------ |

| WARNING | 不要使用锂离子电池作为 RTC 的电源。 |
| --------- | ------------------------------------- |
...
Raspberry Pi Zero, 1, 2 和 3 都有三个 SPI 控制器：

* U 盘，在所有 Raspberry Pis 的引脚上有两个硬件芯片选择信号；还有一种仅适用于 Compute 模块的备用映射。
* U 盘，在除原始 Raspberry Pi 1 Model A 和 Model B 之外的所有 Raspberry Pi 型号上都有三个硬件芯片选择信号。
* SPI2 ，也具有三个硬件芯片选择器，仅在计算模块 1、3 和 3+ 上可用。

在 Raspberry Pi 4、400 和 Compute Module 4 上，还有四个额外的 SPI 总线：SPI3 到 SPI6，每个总线都有两个硬件芯片选择器。这些额外的 SPI 总线可通过某些 GPIO 引脚的替代功能分配来使用。有关更多信息，请参阅 BCM2711 Arm 外围设备数据表。

BCM2835 Arm 外围设备数据表的第 10 章描述了主控制器。第 2.3 章描述了辅助控制器。

#### 引脚/GPIO 映射

##### SPI0

| SPI 功能 | 头部引脚 | Broadcom 引脚名称 | Broadcom 引脚功能 |
| ---------- | ---------- | ------------------- | ------------------- |
...
| SPI 功能 | 标题引脚 | Broadcom 引脚名称 | Broadcom 引脚功能 |
| ---------- | ---------- | ------------------- | ------------------- |
...
LoSSI 标准允许向外围设备（LCD）发出命令并在它们之间传输数据。LoSSI 命令和参数均为 8 位长，但额外的一位用于指示字节是命令还是参数/数据。数据时此额外位设置为高位，命令时设置为低位。最终的 9 位值被串行化输出。LoSSI 常与 MIPI DBI 类型 C 兼容的 LCD 控制器一起使用。

| NOTE | 一些命令会触发 SPI 控制器自动读取，因此该模式不能用作多功能的 9 位 SPI。 |
| ------ | -------------------------------------------------------------------------- |
...
默认情况下，SPI0 已禁用。要启用它，请使用 raspi-config，或确保 dtparam=spi=on 在 /boot/firmware/config.txt 中没有被注释掉。默认情况下，它使用两个芯片选择线，但可以使用 dtoverlay=spi0-1cs 将其减少到一个。还有 dtoverlay=spi0-2cs ；没有任何参数时，它等同于 dtparam=spi=on 。

要启用 SPI1，您可以使用 1、2 或 3 个芯片选择线。将适当的线添加到 /boot/firmware/config.txt 中：
...
驱动程序不使用硬件片选线，因为某些限制。相反，它可以使用任意数量的 GPIO 作为软件/GPIO 片选。这意味着您可以选择任何多余的 GPIO 作为 CS 线，所有这些 SPI 叠加层都包括该控制 - 详细信息请参见 /boot/firmware/overlays/README ，或运行（例如） dtoverlay -h spi0-2cs （可能有助于列出所有这些）。
...
一般来说，Linux 支持的每个设备都可以与 Raspberry Pi 一起使用，尽管在 Raspberry Pi 4 之前的型号上有一些限制。

### 最大功率输出

与所有计算机一样，树莓派上的 USB U 盘提供有限功率。USB 设备通常出现问题是由于电力问题。要排除电力不足导致问题的可能性，请使用带电源的集线器将 USB 设备连接到树莓派上。

| Model          | USB U 盘的最大功率输出                                          |
| ---------------- | ----------------------------------------------------------------- |
| 树莓派 Zero, 1 | 每个 500mA 每port^1^                                            |
| 树莓派 2, 3, 4 | 所有 ports 上的总 1200mA                                        |
| Raspberry Pi 5 | 如果使用 3A 电源，电流为 600mA；如果使用 5A 电源，电流为 1600mA |

1. 对于原始的 Raspberry Pi 1 Model B，限制为每个 port 100mA。

### Raspberry Pi 5

Raspberry Pi 5 需要一款能够提供 3A 的高质量 USB-C 电源适配器在 +5V（15W） 下启动。然而，使用这样的电源适配器会限制外围设备的电流。如果您在首次引导时使用不能提供 +5V 5A 的电源适配器，操作系统会警告您外围设备的电流将受限制为 600mA。

对于希望驱动高功率外设如硬盘和固态硬盘的用户，同时保留峰值工作负载余量，应使用能够提供 5A +5V（25W）的 USB-PD 启用电源供应器。如果树莓派 5 固件检测到这样的电源供应器，它会增加外设的 USB 电流限制至 1.6A，为下游 USB 设备提供额外的 5W 电力，并额外为板上电力预算提供 5W 电力。

| NOTE | 电力预算在 USB 和风扇头之间共享。 |
| ------ | ----------------------------------- |

### 树莓派 4

Raspberry Pi 4 提供两个 USB 3.0 ports 和两个 USB 2.0 ports，它们连接到 VL805 USB 控制器。所有四个 ports 上的 USB 2.0 线连接到 VL805 内的单个 USB 2.0 集线器。这限制了 USB 1.1 和 USB 2.0 设备的总可用带宽与单个 USB 2.0 port 的一致。

在 Raspberry Pi 4 上，先前型号上使用的 USB 控制器位于 USB Type C port，默认情况下被禁用。

### Raspberry Pi Zero，1，2 和 3

树莓派 1 代 B+、树莓派 2 和树莓派 3 主板提供四个 USB 2.0 ports。树莓派 Zero 主板配备一个 micro USB on-the-go (OTG) port。

树莓派 4 之前的型号上的 USB 控制器仅对某些设备提供基本支持，这会带来较高的软件处理开销。它仅支持一个根 USB port：所有连接设备的流量都通过此单一总线传输，该总线的最大速度为 480Mbps。

USB 2.0 规范定义了三种设备速度 - 低速、全速和高速。大多数鼠标和键盘是低速设备，大多数 USB 声音设备是全速设备，而大多数视频设备（网络摄像头或视频捕捉器）是高速设备。

通常，连接多个高速 USB 设备到 Raspberry Pi 没问题。

与低速和全速设备通信时产生的软件开销意味着同时活动的低速和全速设备数量会受限制。连接少量这些类型设备到 Raspberry Pi 不会造成问题。

### 已知的 USB 问题

#### 与 USB 3.0 集线器的互操作性

在使用全速或低速设备（包括大多数鼠标和键盘）时，与 USB 3.0 集线器一起存在问题。 大多数 USB 3.0 集线器硬件中的一个错误意味着在连接到 USB 3.0 集线器的全速或低速设备时，树莓派 4 之前的型号无法与这些设备通信。

当通过 USB 3.0 集线器连接时，USB 2.0 高速设备（包括 USB 2.0 集线器）可正确运行。

避免将低速或全速设备连接到 USB 3.0 集线器。作为解决方法，将 USB 2.0 集线器插入 USB 3.0 集线器的下游port，然后将低速设备连接，或者在树莓派和 USB 3.0 集线器之间使用 USB 2.0 集线器，然后将低速设备插入 USB 2.0 集线器。

#### USB 1.1 网络摄像头

旧的网络摄像头可能是全速设备。因为这些设备传输大量数据并产生额外的软件开销，无法保证可靠运行。作为解决方法，请尝试以较低分辨率使用摄像头。

#### 世异 USB 声卡

昂贵的发烧友级声卡通常使用大量的 USB 带宽。不能保证与 96kHz/192kHz DAC 的可靠操作。作为一种解决方法，强制输出流为 CD 音质（44.1kHz/48kHz 16 位）将带宽降低到可靠水平。

#### 单个 TT USB 集线器

USB 2.0 和 3.0 集线器具有一种机制，用于与连接到其下游 ports 的全速或低速设备通信，称为事务转换器 (TT)。此设备缓冲来自主机的高速请求，并将它们以全速或低速传输给下游设备。USB 规范允许两种集线器配置：单个 TT (所有 ports 共用一个 TT) 和多个 TT (每个 port 一个 TT)。由于硬件限制，如果将太多全速或低速设备插入单个 TT 集线器，这些设备可能会表现不可靠。建议使用多个 TT 集线器与多个全速和低速设备进行接口处理。作为解决方法，将全速和低速设备分散放置在树莓派自己的 USB port 和单个 TT 集线器之间。

## 树莓派修订代码

每个不同的树莓派型号修订版都有一个唯一的修订代码。您可以通过运行以下命令查找树莓派的修订代码：
...
| NOTE | 所有树莓派计算机报告 BCM2835 ，即使是带有 BCM2836、BCM2837、BCM2711 和 BCM2712 处理器的也一样。您不应该使用此字符串来检测处理器。使用下面的信息解码修订代码，或 cat /sys/firmware/devicetree/base/model 。 |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| 0007 | A     | 2.0  | 256MB       | 埃戈曼   |
| 0008 | A     | 2.0  | 256MB       | 索尼英国 |
| 0009 | A     | 2.0  | 256MB       | 奎斯达   |
| 000d | B     | 2.0  | 512MB       | 爱戈曼   |
| 000e | B     | 2.0  | 512MB       | 索尼英国 |
| 000f | B     | 2.0  | 512MB       | Egoman   |
| 0010 | B+    | 1.2  | 512MB       | 索尼英国 |
| 0011 | CM1   | 1.0  | 512MB       | 索尼英国 |
| 0012 | 优秀  | 1.1  | 256MB       | 索尼英国 |
| 0013 | 良好  | 1.2  | 512MB       | 定制     |
| 0014 | CM1   | 1.0  | 512MB       | 定制     |
| 0015 | A+    | 1.1  | 256MB/512MB | 凌狐     |
...
| NOTE | 此列表不是详尽无遗的 - 可能有正在使用但不在此表中的代码。请参阅下一节，了解如何使用修订代码来识别板子的最佳实践。 |
| ------ | ------------------------------------------------------------------------------------------------------------------- |
...
| 900092 | 零                 | 1.2  | 512MB | 索尼英国 |
| 900093 | 零                 | 1.3  | 512MB | 索尼英国 |
| 9000c1 | Zero W             | 1.1  | 512MB | 索尼英国 |
| 9020   | 3A+                | 1.0  | 512MB | 索尼英国 |
| 90200  | 3A+                | 1.1  | 512MB | 索尼英国 |
| 920092 | 零                 | 1.2  | 512MB | 定制     |
| 920093 | 零                 | 1.3  | 512MB | 定制     |
...
| a21041 | 2B                 | 1.1  | 1GB   | 恩贝斯特 |
| a22042 | 2B（搭载 BCM2837） | 1.2  | 1GB   | Embest   |
| a22082 | 3B                 | 1.2  | 1GB   | 定制     |
| a220a0 | CM3                | 1.0  | 1GB   | 恩碧斯特 |
| a32082 | 3B                 | 1.2  | 1GB   | 索尼日本 |
| a52082 | 3B                 | 1.2  | 1GB   | 体育场   |
| a22083 | 3B                 | 1.3  | 1GB   | 定制     |
...
一个天真的实现使用一个支持的版本代码列表，将检测到的代码与该列表进行比较，以决定该设备是否受支持。当出现新的板卡版本或者生产地点发生变化时，这种方法就会失效：每次都会创建一个新的版本代码，而不在支持的版本代码列表中。这将导致同一板卡类型的新版本被拒绝，尽管它们始终保持向后兼容。每当出现新版本时，您都必须发布一个包含新版本代码的新支持的版本代码列表-这是一个繁重的支持负担。

相反，请使用以下方法之一：

* 根据板型字段（3A、4B 等）进行过滤，该字段表示模型但不表示修订版。
* 根据内存量字段进行过滤，因为 RAM 大致对应板的计算能力。

例如，您可以限制支持具有 2GB 或更多 RAM 的树莓派 4B 型号。上一节的示例使用了这种推荐方法。

| NOTE | 始终检查第 23 位，即“新”标志，以确保修订代码是新版本，然后再检查其他字段。 |
| ------ | ------------------------------------------------------------------------------ |

#### 检查树莓派型号和 CPU 跨发行版

在各个 Linux 发行版中， /proc/cpuinfo 的支持和格式会有所不同。要检查树莓派设备在任何 Linux 发行版（包括树莓派 OS）上的型号或 CPU，请检查设备树：
...
输出两个空值分隔的字符串值，每个字符串值包含逗号分隔的制造和型号。例如，Raspberry Pi 5 输出上述板和 CPU 字符串。这些对应以下值：
...
| 树莓派 4 型 A 型        | `raspberrypi`       | `4-model-a`      | `brcm`           | `bcm2711`    |
| 树莓派 4 型 B 型        | `raspberrypi`       | `4-model-b`      | `brcm`           | `bcm2711`    |
| 树莓派计算模块 3        | `raspberrypi`       | `3-compute-module`      | `brcm`           | `bcm2837`    |
| 树莓派 3 型 A+          | `raspberrypi`       | `3-model-a-plus`      | `brcm`           | `bcm2837`    |
| 树莓派 3 型 B+          | `raspberrypi`       | `3-model-b-plus`      | `brcm`           | `bcm2837`    |
| 树莓派 3 型 B           | `raspberrypi`       | `3-model-b`      | `brcm`           | `bcm2837`    |
| 树莓派 2 型 B           | `raspberrypi`       | `2-model-b`      | `brcm`           | `bcm2836`    |
| 树莓派计算模块          | `raspberrypi`       | `compute-module`      | `brcm`           | `bcm2835`    |
| 树莓派 A+型             | `raspberrypi`       | `model-a-plus`      | `brcm`           | `bcm2835`    |
| 树莓派 Model B+         | `raspberrypi`       | `model-b-plus`      | `brcm`           | `bcm2835`    |
| 树莓派 Model B 修订版 2 | `raspberrypi`       | `model-b-rev2`      | `brcm`           | `bcm2835`    |
| 树莓派 Model A          | `raspberrypi`       | `model-a`      | `brcm`           | `bcm2835`    |
| 树莓派 Model B          | `raspberrypi`       | `model-b`      | `brcm`           | `bcm2835`    |
