...
要使用除树莓派官方显示屏之外的其他显示屏，必须在配置文件中指定 dtoverlay 条目。面板制造商应在 Linux 内核代码中配置您的显示屏的定时，并提供一个叠加层来启用这些设置。 请参阅 Adafruit Kippah 显示屏条目以获取示例。 以下示例演示如何在您的 FreeBSD 文件中为 Kippah 显示屏设置一个 dtoverlay 条目：
