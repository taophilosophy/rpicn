...
* 旗舰系列通常被简称为“Raspberry Pi（树莓派）”，搭载高性能硬件、完整的 Linux 操作系统。其外形尺寸与信用卡大小相当，包含了大量常见接口。
* Zero 系列搭载了完整的 Linux 操作系统和比较基本的接口，价格实惠，体积小，功耗低。
* 计算模块系列，通常简称为 "CM"，提供高性能硬件和完整的 Linux 操作系统，适合工业和嵌入式应用的小型形态因素。Compute Module 模型拥有与相应旗舰型号相当的硬件，但较少的ports和没有板载 GPIO 引脚。用户需将 Compute Module 连接到一个提供给定应用所需ports和引脚的独立基板上。
