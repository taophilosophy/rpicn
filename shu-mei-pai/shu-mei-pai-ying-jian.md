...
树莓派的一个强大功能是板的顶部沿着一排 GPIO（通用输入/输出）引脚。所有当前的树莓派板上都有一个 40 针的 GPIO 引脚排针，尽管在树莓派 Zero、树莓派 Zero W 和树莓派 Zero 2 W 上是未焊接的。所有板上的 GPIO 引脚都具有 0.1 英寸（2.54 毫米）的引脚间距。
...
-d 标志指示 rpi-eeprom-update 使用指定的映像文件中的配置，而不是自动迁移当前配置。
...
* 如果引导加载程序更新映像称为 pieeprom.upd ，则更新完成后， recovery.bin 将重命名为 recovery.000 ，然后系统将重新启动。由于 recovery.bin 不再存在，ROM 将从 SPI 闪存加载新更新的引导加载程序，操作系统将正常启动。
* 如果引导加载程序更新映像称为 pieeprom.bin ，则更新完成后 recovery.bin 将停止。成功后，HDMI 输出将变为绿色，绿色活动指示灯将快速闪烁。如果更新失败，HDMI 输出将变为红色，并通过活动指示灯显示错误代码。
...
接下来，将合适的 USB 串行电缆连接到您的主机计算机（树莓派将起作用，尽管您可能会发现，最简单的方法是使用 USB 串行电缆，因为它可以在不需要任何烦人的 config.txt 设置的情况下立即使用）。在树莓派或计算模块上使用标准引脚 6、8 和 10（GND、GPIO14、GPIO15）。
...
* 默认情况下，连接到 3A 电源时禁用 USB 启动。在 /boot/firmware/config.txt 中设置 usb_max_current_enable=1 以启用 USB 启动。或者，您可以在 USB 启动失败时单击一次电源按钮，临时启用 usb_max_current_enable 并继续启动。但是，如果通过按电源按钮启用，此设置在重新启动后不会持续。
...
自 2014 年树莓派 B+（除 Zero 系列外）的所有型号上，都有低电压检测电路，可检测供电电压是否低于 4.63V（±5%）。这将导致内核日志中添加一个条目。
...
一个天真的实现使用支持的修订代码列表，将检测到的代码与列表进行比较，以决定设备是否受支持。当出现新的板卡版本或生产地点更改时，这种方法会中断：每次出现新的修订代码时，都会在支持的修订代码列表中创建一个新的修订代码。这将导致同一板卡类型的新修订版本被拒绝，尽管它们始终向后兼容。每次出现新的修订版本时，您都必须发布一个包含新修订代码的新支持的修订代码列表 - 这是一个繁重的支持负担。
