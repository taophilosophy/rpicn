...
请将电源适配器插入树莓派（该接口标有“POWER IN”、“PWR IN”或“PWR”）。某些型号的树莓派（如 Zero 系列），其 USB 输出接口的规格与电源接口相同。请确保你在树莓派上所使用的接口是正确的！
...
自树莓派 1A+ 以来，所有消费者型号的树莓派都搭载了 microSD 卡槽。在卡槽中插入 microSD 后，你的树莓派就能自动从 microSD 启动。
