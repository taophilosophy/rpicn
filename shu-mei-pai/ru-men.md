...
我们建议，用于安装树莓派系统（Raspberry Pi OS）的存储卡，存储容量至少为 32GB（对于精简版树莓派系统（Raspberry Pi OS Lite），建议至少为 16GB）。你可以使用任何容量不大于 2TB 的存储卡。由于 MBR 的限制，目前不支持大于 2TB 的容量。同所有其他启动介质一样，读写速度更快的存储卡性能也更佳。
