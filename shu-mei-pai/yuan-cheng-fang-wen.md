...
分配给连接到局域网的任意设备一个 IP 地址。要使用 SSH 或 VNC 从另一台机器连接到树莓派，你需要知道树莓派的 IP 地址。如果连接了显示器，这很容易，还有许多方法可以从网络上的另一台机器远程查找它。
...
如果你的设备支持 mDNS，你可以通过使用其主机名和 .local 后缀来访问你的树莓派。在新安装的树莓派系统上，默认主机名为 raspberrypi，因此默认情况下，所有运行树莓派系统的树莓派都会响应：
