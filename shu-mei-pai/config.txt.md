...
在 CEA 和 DMT 标准中的信号格式，仅有一种无法兼容：`DMT mode 81, 1366x768 @ 60Hz`。该信号格式的水平同步和下沿时序值为奇数，宽度无法被 8 整除。
...
在默认情况下，会输出复合视频（NTSC）。要选择其他信号格式，请将以下内容添加到 `/boot/firmware/cmdline.txt` 中这一行：
