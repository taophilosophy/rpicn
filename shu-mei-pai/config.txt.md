...
在默认情况下，固件会解析所有接入 HDMI 显示器的 EDID，选择适当的视频模式，然后通过内核命令行上的设置将模式的分辨率和帧率（以及过扫描参数）传给 Linux 内核。在罕见情况下，固件可能会选择 EDID 中不存在的模式，这可能与设备并不兼容。使用 `disable_fw_kms_setup=1` 可以禁用传递视频模式参数，从而避免此问题。Linux 视频模式系统（KMS）会自己解析 EDID 并选择适当的模式。
