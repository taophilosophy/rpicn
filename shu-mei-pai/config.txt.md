...
| NOTE | 树莓派 5 固件内置在引导加载程序 EEPROM 中。|
| ------ | --------------------------------------------- |
...
| NOTE | 64 位内核可以是未压缩的镜像文件和镜像的 gzip 压缩文件（仍可称为 kernel8.img；引导加载程序将从开头的签名 B 识别存档）。64 位内核仅适用于树莓派 3、3+、4、400、Zero 2 W 和 2B 修订版 1.2，以及树莓派计算模块 3、3+ 和 4。树莓派 5 仅支持 64 位内核，因此对该设备来说，此参数无效。|
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
由于 树莓派 4B 和 400 上没有专用 GPIO 跳线帽，必须使用其他 GPIO 来选择 RPIBOOT 模式，将 GPIO 拉低。从以下选项中选择一个单个 GPIO:
...
| WARNING | 将任何超频参数设置为非 raspi-config 使用的值可能会在 SoC 内部置一个永久位。这将使你的 树莓派 曾经超频的情况变得可检测。当将 force_turbo 设置为 `1` 且任一 `over_voltage_*` 选项设为大于 `0` 的值时，超频位会设置。详细信息请查看有关 Turbo 模式的博文。|
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| NOTE | 某些的型号的树莓派（Zero W、Zero 2 W、3B+，树莓派 400，计算模块 4、4S）查看多个过滤器的设置（如上表所列）。这意味着如果你想让一个设置只适用于（例如）4B 型号，而不会将该设置应用于 Pi 400，那么 [pi4] 部分中的设置需要在随后的 [pi400] 部分中通过替代设置来恢复 - 这些部分的顺序很重要。或者，你可以使用 [board-type=0x11] 过滤器，这个过滤器与不同的硬件产品之间有一对一的映射关系。|
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
