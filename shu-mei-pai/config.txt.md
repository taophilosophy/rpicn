...
内核默认启用了驱动程序 CPUFreq，在安装 raspi-config 后，会在启动时把调度程序 powersave 切换成 ondemand。通过调度程序 ondemand，CPU 频率将随处理器负载而变化。你可以通过把配置参数 `*_min` 调整最小值，或通过应用静态缩放调度程序（powersave、performance），亦或使用 force_turbo=1 来禁用动态频率调整。
...
| `[pi3+]`       | 3A+、3B+（也引用 [pi3] 的内容）                            |
| `[pi4]`       | 4B、400，计算模块 4、4S                    |
| `[pi5]`       | 树莓派 5                                                                 |
| `[pi400]`       | Pi 400（也引用 [pi4] 内容）                                              |
| `[cm4]`       |计算模块 4（也引用 [pi4] 内容）                                          |
| `[cm4s]`       |计算模块 4S（也引用 [pi4] 内容）                                         |
| `[pi0]`       | Zero、Zero W、Zero 2 W                                                   |
| `[pi0w]`       | Zero W（也引用 [pi0] 内容）                                              |
| `[pi02]`       | Zero 2W（也引用 [pi0w] 和 [pi0] 的内容）                               |
...
输出的底部附近将显示一个 16 位十六进制值。你树莓派的串号是最后八位十六进制数字。例如，如果你看到：
...
设置为 1 启用复合视频输出，设置为 0 禁用。在树莓派 4、5 上，仅当你将其设置为 1 时，才可使用复合输出（同时也会禁用 HDMI 输出）。在树莓派 400 上不支持复合输出。
...
除了树莓派 4、5 以外，都需禁用 HDMI 输出才能启用复合输出。当未连接（未检测到）HDMI 显示器时，HDMI 输出被禁用。设置 enable_tvout=0 可阻止在禁用 HDMI 时启用复合输出。
