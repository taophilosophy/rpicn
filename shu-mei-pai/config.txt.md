...
>**注意**
>
>使用该配置参数（`start*cd.elf`、`fixup*cd.dat`）并不能调用精简固件，只会导致系统无法启动。唯一能启用精简固件的方法是指定 `gpu_mem=16`。精简固件删除了对编解码器、3D 和调试日志的支持，以及把初始早期引导帧缓冲区限制到 1080p @16bpp——正如同其他固件那样，KMS 在后续阶段可使用高达 32bpp 4K 帧缓冲区代替之。
