...
在默认情况下，当接入 4K 显示器，树莓派 4B、400 和 CM4 将使用 30Hz 刷新率。使用此选项可使用 60Hz 的刷新率。树莓派 4 无法同时在两个 micro HDMI 接口上输出 4Kp60。设置 `hdmi_enable_4kp60` 会增加功耗和温度。

### 常见硬件配置选项

#### `camera_auto_detect`

启用此设置（树莓派系统中默认启用），固件将自动加载所识别的 CSI 摄像头叠加层。如 `camera_auto_detect=0` 则表示禁用该设置。
...
`dtoverlay` 选项要求固件加载指定的设备树覆盖层——这是配置文件，可启用内、外围硬件的内核支持。例如，`dtoverlay=vc4-kms-v3d` 会加载叠加层，启用内核图形驱动程序。
...
树莓派的设备树配置文件可支持很多参数（如启用 I²C 和 SPI 接口）。大部分 DT 叠加层都可通过参数进行配置。这两种类型的参数都可以使用 `dtparam` 进行设置。此外，可以把叠加层参数添加到选项 `dtoverlay` 中，以逗号分隔，但请注意字符长度被限制为 98 个字符。

欲了解更多详情，请查阅 DTB、叠加层和 `config.txt`。

#### `arm_boost`（仅适用于树莓派 4）

所有树莓派 400 系列和树莓派 4B 的修订版都搭载了另一个开关电源（SMPS）（用于 SoC 电压轨），这使默认超频模式下的主频，可从 1.5GHz 增加到 1.8GHz。此更改在树莓派系统下默认生效。设置 `arm_boost=0` 可禁用。
...
`audio_pwm_mode=1` 使用 3.5 mm AV 接口的传统低质量模拟音频。

`audio_pwm_mode=2`（默认）使用先进的调制方案选择高质量模拟音频。

>**注意**
>
>此选项会占用更多的 GPU 计算资源，可能会对某些型号的特定用例造成干扰。
...
在音频样本大于 16 位时，通常会禁用音频抖动（请参阅上述 `disable_audio_dither`）。将此选项置为 `1` 可强制对所有位深度应用抖动。

### `pwm_sample_bits`

`pwm_sample_bits` 命令能调整模拟音频输出的位深度。默认位深度为 `11`。设置低于 `8` 的位深度将导致音频无法正常工作，因为低于 `8` 的值会导致 PLL 频率过低：对于设备来说，是不可能的。该参数通常仅用于演示位深度对量化噪声的影响。
...
`cmdline` 是启动分区中的可选文件名（默认值为 `cmdline.txt`），用于读取内核命令行字符串。
...
在 64 位模式下，固件将选择适当的内核（例如 `kernel8.img`），除非显式设置了 `kernel` 参数，否则将使用该选项。

在树莓派 4 系列（4B、400、CM4 和 CM4S）上默认为 1，在所有其他平台上默认为 0。但是，如果显式设置的 `kernel` 选项中给定的名称与已知的任一内核匹配，则 `arm_64bit` 将应用相应设置。
...
在默认情况下，启动时会在 I²C 总线上进行探测（哪怕未接入 PoE 扩展板）。将此参数置为 1，可禁用在 I²C（在 `ID_SD＆ID_SC` 引脚）上对 PoE 扩展板风扇的检测行为。如果你不打算使用 PoE 扩展板，那么这是最大程度上缩短启动时间的有效方式。
...
[all] 筛选器是最基本的筛选器。它重置所有先前设置的筛选器，并允许将下面列出的任何设置应用于所有硬件。通常最好在筛选设置组的末尾添加一个 [all] 筛选器，以避免意外组合筛选器（请参见下文）。
...
[pi2]
# 这些设置仅在接入 VSC-TD2220 显示器，且使用树莓派 2 时生效
...
将 `awb_auto_is_greyworld` 置为 `1` 能让那些内部不支持灰色世界选项的库或应用程序捕捉使用夜视（NoIR）相机拍摄的有效图像和视频。它将自动 awb 模式切换为使用灰色世界算法。仅夜视相机需要，或者高质量相机在移除红外滤镜后，才需要这样做。
