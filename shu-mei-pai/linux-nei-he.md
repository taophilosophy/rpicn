...
树莓派内核托管在 GitHub 上；更新滞后于上游 Linux 内核。上游内核持续更新，我们采用 LTS 版本的内核，这些版本在首页上有所提及，并将修改集成到我们自己的树莓派内核中。然后我们会创建一个包含内核不稳定端口的“next”分支，经过大量测试和讨论后，我们将其推送到我们存储库的主分支。

### 更新你的内核

如果你使用标准的树莓派系统更新和升级过程，这将自动将内核更新到我们内核的最新稳定版本。这是推荐的步骤。但是，在某些情况下，你可能希望更新到最新的“激进”或测试内核。只有在树莓派工程师建议这样做，或者最新软件中有特定功能时，你才应该这样做。

### 将你的代码放入内核

由于许多原因，你可能希望将某些内容放入内核中：
...
其中 `<branch>` 是你希望下载的分支的名称。

有关可用分支的信息，请参考原始 GitHub 存储库。

#### 内核配置

配置内核；除了默认配置外，你可能希望更详细地配置内核或从其他来源应用补丁，以添加或删除所需功能。

##### 应用默认配置

首先，通过运行以下命令准备默认配置，具体取决于你树莓派的型号。

对于树莓派 1，Zero、Zero W，以及树莓派计算模块 1 默认（仅 32 位）构建配置：
...
适用于树莓派 2、3、3+，Zero 2 W，以及树莓派计算模块 3 和 3+ 默认的 32 位构建配置：
...
适用于树莓派 4、400，以及树莓派计算模块 4 默认的 32 位构建配置：
...
适用于树莓派 3、3+、4、400，Zero 2 W，以及树莓派计算模块 3、3+、4 默认的 64 位构建配置：
...
内核编译完成后，你需要将其复制到你的树莓派上并安装模块。最好直接使用 SD 卡阅读器完成此操作。

首先，在插入 SD 卡之前和之后使用 lsblk 来识别它。你最终应该得到类似这样的东西：
...
配置通常通过 make menuconfig 界面完成。或者，你可以手动修改 .config 文件，但这对新用户来说可能更困难。
...
menuconfig 实用程序具有简单的键盘导航。经过简短的编译后，你将看到一个包含所有可配置选项的子菜单列表。有许多选项，所以请花些时间阅读并熟悉它们。

使用方向键进行导航，使用回车键进入子菜单（由 ---> 指示），连续按两次 Esc 键返回上一级或退出，使用空格键循环更改选项状态。某些选项有多个选择，这种情况下它们将显示为子菜单，按回车键将选择一个选项。你可以在大多数条目上按 h 键获取有关特定选项或菜单的帮助。
...
在构建自定义内核时，你可能希望将补丁或补丁集（patchsets）应用于 Linux 内核。
...
在这种情况下，这些源适用于 6.1.38 内核。你可以使用 uname -r 命令查看系统上运行的版本。
...
如果你正在编译内核模块或类似内容，你将需要 Linux 内核头文件。这些文件在编译与内核进行接口的代码时提供所需的各种函数和结构定义。

如果你从 GitHub 克隆了整个内核，头文件已经包含在源代码树中。如果你不需要所有额外的文件，可以仅从树莓派系统存储库安装内核头文件。

如果你正在使用树莓派系统的 32 位版本，请运行：

```
 sudo apt install linux-headers-rpi-{v6,v7,v7l}
```

或者，如果你正在使用树莓派系统的 64 位版本，请运行：
...
>**注意**
>
>由于将安装许多小文件，完成此命令可能需要相当长的时间。没有进度指示器。

当发布新内核版本时，你将需要与该内核版本匹配的头文件。仓库更新以同步最新内核版本可能需要几周时间。如果发生这种情况，最好的方法是克隆内核。
