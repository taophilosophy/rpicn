...
首先，您应该分叉 Linux 存储库并在构建系统上克隆该存储库；这可以是您的树莓派上的系统，也可以是您用于交叉编译的 Linux 机器。然后，您可以进行更改、测试并将其提交到您的分叉存储库中。
...
>**注意**
>
>在树莓派 2、3、4 和 5 上， -j4 标志将工作分配给所有四个核心，显著加快编译速度。

如果您现在重新启动，您的树莓派应该正在运行您新编译的内核。
...
构建内核后，您需要将其复制到您的树莓派上并安装模块。最好直接使用 SD 卡阅读器完成此操作。
...
>**注意**
>
>由于安装了许多小文件，此命令完成可能需要相当长的时间。没有进度指示器。
